...
$f(x + T) = f(x),$
那么函数  $f(x)$  就叫做周期函数(periodicfunction).非零常数  $T$  叫做这个函数的周期(period).
周期函数的周期不止一个.例如,  $2\pi$  ,  $4\pi$  ,  $6\pi$  ,…以及  $- 2\pi$  ,  $- 4\pi$  ,  $- 6\pi$  ,…都是正弦函数的周期.事实上,  $\forall k\in \mathbf{Z}$  且  $k\neq 0$  ,常数  $2k\pi$  都是它的周期.
...
所以  $\cos 2(x + \pi) = \cos 2x$ ,  $x \in \mathbf{R}$ 。
