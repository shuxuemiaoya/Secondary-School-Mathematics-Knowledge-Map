...
<span class="fake-tag">解析</span>由题意， $f(x)$ 的最小正周期 $T = \frac{2\pi}{2} = \pi$ ， $f(x) \leq f\left(\frac{2\pi}{9}\right) \Rightarrow f(x)$ 在 $x = \frac{2\pi}{9}$ 处取得最大值，
...
$\quad$A. $\left[\frac{3\pi}{8}, \frac{\pi}{2}\right]$

$\quad$B. $\left[\frac{\pi}{2},\frac{3\pi}{4}\right]$

$\quad$C. $\left[\frac{\pi}{2},\frac{3\pi}{2}\right]$

$\quad$D. $\left[\frac{3\pi}{4}, \frac{3\pi}{2}\right]$

<span class="fake-tag">解析</span>由题意， $f(x)=\sqrt{2}\sin\left(x+\frac{\pi}{4}\right)$ ，给了值域，考虑画图分析，为了便于作图，先将 $x+\frac{\pi}{4}$ 换元成t，
...
<span class="fake-tag">解析</span>由题意， $f(x) = 2\sin \left(2x + \frac{\pi}{3}\right) + \cos \left(2x + \frac{\pi}{3}\right) = \sqrt{5}\sin \left(2x + \frac{\pi}{3} +\varphi\right),$
...
<span class="fake-tag">解析</span>条件中有相邻的对称轴和零点，它们之间的距离为 $\frac{T}{4}$ ，故可由此求出周期，再得到 $\omega$ ，
...
$\quad$A. $\omega = \frac{1}{3},\varphi = -\frac{11\pi}{24}$

$\quad$B. $\omega = \frac{2}{3}, \varphi = \frac{\pi}{12}$

$\quad$C. $\omega = \frac{1}{3},\varphi = \frac{7\pi}{24}$

$\quad$D. $\omega = \frac{2}{3}, \varphi = -\frac{11\pi}{12}$

<span class="fake-tag">解析</span> $f\left(\frac{5\pi}{8}\right) = \frac{1}{2} \Rightarrow x = \frac{5\pi}{8}$ 是 $f(x)$ 的最大值点， $f\left(\frac{11\pi}{8}\right) = 0 \Rightarrow x = \frac{11\pi}{8}$ 是 $f(x)$ 的零点，
...
$\quad$A. 转动 $10 \mathrm{~min}$ 后点 $P$ 距离地面 $8 \mathrm{~m}$   
$\quad$B. 若摩天轮转速减半, 则转动一圈所需的时间变为原来的 $\frac{1}{2}$   
$\quad$C. 第 $17 \mathrm{~min}$ 和第 $42 \mathrm{~min}$ 点 $P$ 距离地面的高度相同  
$\quad$D. 摩天轮转动一圈, 点 $P$ 距离地面的高度不低于 $85 \mathrm{~m}$ 的时间长为 $\frac{20}{3} \mathrm{~min}$

![](images/b327881a6d3034efb5e907e076dea631c556eff915dd11d5d1c4419756a05ade.jpg)

<span class="fake-tag">解析</span>: 诸多选项涉及点 $P$ 距离地面高度和时间, 故先建立二者的函数关系, 可设该高度为 $f(t) = A \sin (\omega t + \varphi) + B$ , 其中 $A > 0$ , $\omega > 0$ . 只要求出 $A, B, \omega, \varphi$ , 解析式就有了, 先由最大、最小值求 $A$ 和 $B$ ,由题意，点 $P$ 最高为 $110\mathrm{m}$ ，最低为 $10\mathrm{m}$ ，所以 $\left\{ \begin{array}{l} A + B = 110 \\ -A + B = 10 \end{array} \right.$ ，解得： $A = 50$ ， $B = 60$ ，再由初始位置求 $\varphi$ ，初始位置在最高点，所以 $f(0) = 50\sin \varphi +60 = 110$ ，从而 $\sin \varphi = 1$ ，故 $\varphi = \frac{\pi}{2}$ 最后由周期求 $\omega$ ，由题意， $20\mathrm{min}$ 转一圈，所以 $T = 20$ ，故 $\omega = \frac{2\pi}{T} = \frac{\pi}{10}$
...
$\quad$A. 水深 $y$ （单位：米）与时间 $x$ （单位：时）的函数关系为 $y = 3\sin \left(\frac{\pi}{6} x + \frac{\pi}{6}\right) + 7$ ， $x \in [0,24)$   
$\quad$B. 该船满载货物时可以在0:00到4:00之间以及12:00到16:00之间进入港口  
$\quad$C. 该船卸完货物后可以在 19:00 离开港口  
$\quad$D. 该船5月10日完成卸货任务的最早时间为16:00
