...
<span class="fake-tag">解析</span>由题意， $f(x)$ 的最小正周期 $T = \frac{2\pi}{\omega} = \pi \Rightarrow \omega = 2$
...
<span class="fake-tag">解析</span>先写出 $f(x)$ 的解析式并化简，
...
<span class="fake-tag">解析</span>A项，函数 $y = \tan (\omega x + \varphi)$ 的最小正周期为 $\frac{\pi}{|\omega|}$ ，所以 $f(x)$ 的最小正周期 $T = \frac{\pi}{2}$ ，故A项正确；
...
$\quad$C. $f(x)$ 在 $\left(-\frac{5\pi}{12}, \frac{5\pi}{6}\right)$ 内有 3 个极值点  
...
<span class="fake-tag">解析</span>直接用 $f(x)$ 的图象来分析不方便，可将 $\omega x + \frac{3\pi}{4}$ 换元成 $t$ ，转化为用 $y = \cos t$ 的图象来看，
...
<span class="fake-tag">解析</span>为了便于分析，先将 $\omega x - \frac{\pi}{6}$ 换元成 $t$ ，用 $y = \sin t$ 的图象来研究问题，设 $t = \omega x - \frac{\pi}{6}$ ，则 $f(x) = \sin t$
...
<span class="fake-tag">解析</span>: 可发现换元后的两边端点值均不固定且不过定值, 故考虑用例 3 变式 4 解法 2 的通法, 先求 $f(x)$ 的减区间, 再由 $\left[\frac{\pi}{2}, \pi\right]$ 包含于该减区间建立不等式求 $\omega$ 的范围,
...
<span class="fake-tag">解析</span>直接画 $f(x)$ 的图象来分析不便，可先将 $\omega x - \frac{\pi}{6}$ 换元成 $t$ ，用 $y = \sin t$ 的图象来分析问题，
...
<span class="fake-tag">解析</span>本题没有规定 $\omega$ 的正负，但结合选项，可以只考虑 $\omega > 0$ 的情形，但这里我们把 $\omega \leq 0$ 的情况也来做个分析，弄明白为什么 $\omega \leq 0$ 不满足题意，
...
<span class="fake-tag">解析</span>由题意， $T=\frac{2\pi}{\omega}$ ， $f(T)=\cos\left(\omega\cdot\frac{2\pi}{\omega}+\varphi\right)$
...
<span class="fake-tag">解析</span>要求 $f\left(\frac{\pi}{2}\right)$ ，应先求 $\omega$ 和 $b$ ，题干给出了 $T$ 的范围，可由此找到 $\omega$ 的范围，
...
<span class="fake-tag">解析</span> $T=\frac{2\pi}{3}\Rightarrow\omega=\frac{2\pi}{T}=3$ ，所以 $f(x)=\cos(3x+\varphi)$ ，
