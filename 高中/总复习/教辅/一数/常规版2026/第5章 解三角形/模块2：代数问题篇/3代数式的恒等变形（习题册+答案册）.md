...
<span class="fake-tag">解析</span>若将 $\sin C = 2\sin A$ 角化边，则结合 $2b^{2} - 3c^{2} - ac = 0$ 可将边统一起来，由余弦定理推论求 $\cos C$
...
<span class="fake-tag">解析</span>题干涉及两边与三内角正弦，要求第三边，考虑正弦定理.若不知道怎么求，就都写出来再看，
...
<span class="fake-tag">解析</span>因为 $\sin (C - B) = 2\sin B\cos C$
...
<span class="fake-tag">解析</span>让求的是角，故可考虑将边的齐次式 $b^{2} = \frac{9}{4} ac$ 化角分析，因为 $b^{2} = \frac{9}{4} ac$ ，所以 $\sin^2 B = \frac{9}{4}\sin A\sin C$ ，
