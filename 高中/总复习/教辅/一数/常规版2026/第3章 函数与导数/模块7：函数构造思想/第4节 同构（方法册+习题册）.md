...
<span class="fake-tag">解析</span>右边有 $a^3$ ，左边可变形出 $\left(\frac{2}{b + 1}\right)^3$ ，故考虑看看左右是否同构，稍作变形就会发现是的，
...
<span class="fake-tag">解析</span>所给的两个等式都无法直接求出 $a$ 和 $b$ ，但形式相近，故考虑同构，其中 $a = \mathrm{e}^{5 - a}$ 这个式子已经很简单了，所以将 $2 + \ln b = \mathrm{e}^{3 - \ln b}$ 朝 $a = \mathrm{e}^{5 - a}$ 变形，对比此两式发现可将 $2 + \ln b$ 整体换元成 $t$ ，
...
<span class="fake-tag">答案</span>: $e^{3}$

【变式 2】若关于 x 的不等式 $e^{ax} + e^{-\ln x} - \sin(\ln x) > e^{-ax} + e^{\ln x} - \sin(ax)$ 在区间 $(0, +\infty)$ 上恒成立，则实数 a 的取值范围是（）

$\quad$A. $\left(\frac{2}{\mathrm{e}}, +\infty\right)$

$\quad$B. $\left(\frac{1}{\mathrm{e}}, +\infty\right)$

$\quad$C. $\left(-\infty, \frac{1}{\mathrm{e}}\right)$

$\quad$D. $\left(-\infty, \frac{2}{\mathrm{e}}\right)$

<span class="fake-tag">解析</span>不等式两端有些相似，考虑通过同构将其化简，要同构，先把含 $a$ 的和不含 $a$ 的分离到两端， $\mathrm{e}^{ax} + \mathrm{e}^{-\ln x} - \sin (\ln x) > \mathrm{e}^{-ax} + \mathrm{e}^{\ln x} - \sin (ax) \Leftrightarrow \mathrm{e}^{ax} - \mathrm{e}^{-ax} + \sin (ax) > \mathrm{e}^{\ln x} - \mathrm{e}^{-\ln x} + \sin (\ln x)$ ①，
...
<span class="fake-tag">解析</span>若将 $x \mathrm{e}^{x}$ 调整为 $\mathrm{e}^{\ln x + x}$ ，则含 $x$ 的部分都以 $x + \ln x$ 这一整体结构出现，可换元简化不等式，
...
<span class="fake-tag">解析</span>观察发现参数 $a$ 不易分离，也无法像例2那样化整体结构换元，怎么办呢？注意到有 $\ln (x + a)$ ，故先两端同时加 $x$ ，将右侧统一成 $x + a$ ，就会发现可按 $\mathrm{e}^x + x$ 与 $x + \ln x$ 的同构方法来处理，
...
<span class="fake-tag">解析</span>看到 $\lambda \mathrm{e}^{\lambda x}$ 这一结构，想到乘以 $x$ 将前面的系数 $\lambda$ 调整为与指数部分一致，
...
设 $f(x) = x\mathrm{e}^{x}(x\in \mathbf{R})$ ，则 $f'(x) = (x + 1)\mathrm{e}^{x}$ ，所以 $f'(x) > 0\Leftrightarrow x > - 1$ ， $f'(x) <   0\Leftrightarrow x <   - 1$
...
<span class="fake-tag">答案</span> $\frac{1}{\mathrm{e}}$
...
设 $h(x) = \mathrm{e}^{x} + x(x\in \mathbf{R})$ ，则 $h'(x) = \mathrm{e}^{x} + 1 > 0$ ，所以 $h(x)$ 在 $\mathbf{R}$ 上单调递增，
...
$\quad$A. $[0,\mathrm{e}]$ 
$\quad$B. $\left[-\mathrm{e}^{1 - \frac{1}{\mathrm{e}}},\mathrm{e}\right]$

$\quad$C. $\left(-\infty, -\mathrm{e}^{1 - \frac{1}{\mathrm{e}}}\right]\bigcup [\mathrm{e}, +\infty)$ 
