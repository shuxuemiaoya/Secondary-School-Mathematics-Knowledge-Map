...
<span class="fake-tag">解析</span>注意到(1,2)为开区间，最值不可能在端点取，所以要使 $f(x)$ 在(1,2)上有最小值，则最小值一定在区间内部取得，可导函数区间内部的最值必为极值，于是求导分析 $f(x)$ 的极值情况，

由题意， $f'(x) = x^{2} + \mathrm{e}^{x - 1} + m$ ，因为在(1,2)上， $y = x^{2}$ 和
...
此时 $f'(x)$ 在(1,2)上有零点 $x_0$ ，且由图可知当 $x\in (1,x_0)$ 时， $f'(x) <   0$ ，当 $x\in (x_0,2)$ 时， $f'(x) > 0$
...
<span class="fake-tag">解析</span>先将参数 $b$ 全分离出来，便于作图研究交点， $g(x) = 0 \Leftrightarrow f(x) - b = 0 \Leftrightarrow f(x) = b$
...
<span class="fake-tag">解析</span>先全分离，转化为水平直线与函数图象交点问题， $f(x) = -\frac{1}{4} x + a \Leftrightarrow f(x) + \frac{1}{4} x = a$ ，
...
<span class="fake-tag">解析</span>观察可得 $g(x)$ 为奇函数，可先用对称性将研究的范围缩小，由题意， $g(-x) = f(-x) - f(x) = -g(x)$ ，
...
<span class="fake-tag">解析</span>先将 $f(x) = 0$ 半分离， $f(x) = 0 \Leftrightarrow ax = \ln x$ ，注意
