...
<span class="fake-tag">解析</span> $f(f(x)) < 2$ 为双层复合结构的不等式，可考虑通过将 $f(x)$ 换元，化整为零，来简化该不等式，
...
<span class="fake-tag">解析</span> $f(f(x)) - f(x) + 1$ 是复合结构，它由 $y = f(t) - t + 1$ 和 $t = f(x)$ 复合而成，所以先将 $f(x)$ 换元，化整为零，设 $t = f(x)$ ，则 $f(f(x)) - f(x) + 1 \leq 0$ 即为 $f(t) - t + 1 \leq 0$ ，
...
<span class="fake-tag">解析</span>先将 $f(g(x)) \geq 1$ 内层的 $g(x)$ 换元，化整为零，
