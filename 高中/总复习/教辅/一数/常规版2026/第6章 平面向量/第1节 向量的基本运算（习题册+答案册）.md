...
<span class="fake-tag">解析</span>题干给出了与 $A, B, C, D$ 四点有关的向量，故考虑用向量共线来判断三点共线，
...
<span class="fake-tag">解析</span>条件有两个模的等式，想到把它们平方，观察联系，
...
$\quad$A. $\left\| \pmb {a}\right| - \left|\pmb {b}\right|\leq \left|\pmb {a} + \pmb {b}\right|\leq \left|\pmb {a}\right| + \left|\pmb {b}\right|$

$\quad$B. 若 $a, b$ 为非零向量，且 $\vert a + b\vert = \vert a - b\vert$ ，则 $a \perp b$
...
<span class="fake-tag">解析</span>涉及模的问题，考虑将其平方来看，
...
<span class="fake-tag">解析</span>算向量夹角，考虑夹角余弦公式 $\cos < a, b > = \frac{a \cdot b}{|a| \cdot |b|}$ 。用数量积翻译两个垂直条件，可建立 $a \cdot b$ 和 $|a|$ ， $|b|$ 的关系， $\left\{ \begin{array}{l} a \perp (a + 4b) \\ b \perp (a + 3b) \end{array} \right. \Rightarrow \left\{ \begin{array}{l} a \cdot (a + 4b) = a^2 + 4a \cdot b = |a|^2 + 4a \cdot b = 0 \\ b \cdot (a + 3b) = a \cdot b + 3b^2 = a \cdot b + 3|b|^2 = 0 \end{array} \right.$ ，观察发现①②都有 $a \cdot b$ ，消去 $a \cdot b$ ，即可得到 $|a|$ 与 $|b|$ 的关系，由②得 $a \cdot b = -3|b|^2$ ，代入①得 $|a|^2 + 4 \times (-3|b|^2) = 0$ ，
...
<span class="fake-tag">解析</span>所求式可化为 $a \cdot b + a \cdot c$ ，怎样计算它们？观察发现将所给的向量等式移项，平方即可产生 $a \cdot b$ 和 $a \cdot c$
...
<span class="fake-tag">解析</span>如图， $\overrightarrow{DE} = \overrightarrow{DC} +\overrightarrow{CE} = -\frac{1}{2}\overrightarrow{CA} +\frac{3}{2}\overrightarrow{CB} = -\frac{1}{2}\pmb {a} + \frac{3}{2}\pmb {b}$
