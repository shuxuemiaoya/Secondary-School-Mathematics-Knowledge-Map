...
<span class="fake-tag">解析</span>由题意， $z = \frac{1 - \mathrm{i}}{2 + 2\mathrm{i}} = \frac{(1 - \mathrm{i})(2 - 2\mathrm{i})}{(2 + 2\mathrm{i})(2 - 2\mathrm{i})} = \frac{2 - 2\mathrm{i} - 2\mathrm{i} + 2\mathrm{i}^2}{4 - 4\mathrm{i}^2}$
...
<span class="fake-tag">解析</span> $\frac{z}{z - 1} = 1 + \mathrm{i}\Rightarrow z = (1 + \mathrm{i})z - (1 + \mathrm{i})\Rightarrow \mathrm{i}z = 1 + \mathrm{i},$
...
<span class="fake-tag">解析</span> $(1 + 5\mathrm{i})\mathrm{i} = \mathrm{i} + 5\mathrm{i}^2 = \mathrm{i} - 5 = -5 + \mathrm{i}$
...
<span class="fake-tag">解析</span>由 $\frac{z}{3 - \mathrm{i}} = 1 + \mathrm{i}$ 可得 $z = (3 - \mathrm{i})(1 + \mathrm{i}) = 3 + 3\mathrm{i} - \mathrm{i} - \mathrm{i}^2$
...
<span class="fake-tag">解析</span>设 $z = a + b\mathrm{i}(a,b\in \mathbf{R})$ ，则 $z - \mathrm{i} = a + (b - 1)\mathrm{i}\in \mathbf{R}$
...
<span class="fake-tag">解析</span>由 $|z - 3 + 4\mathrm{i}| = 1$ 不方便直接求出 $z$ ，故考虑设 $z$ 的代数形式，翻译已知条件来分析，
...
<span class="fake-tag">解析</span>由所给等式不方便直接分离出 $z$ ，故考虑设 $z$ 的代数形式，代入所给等式，
...
<span class="fake-tag">解析</span>A项，复数的平方可能是虚数，而复数的模是实数，平方后仍为实数，故 $z^2$ 与 $\left|z\right|^2$ 不一定相等，下面举个反例，
...
<span class="fake-tag">解析</span>怎样翻译 $z^2 = (\overline{z})^2$ ？若无思路，不妨设复数 $z$ 的代数形式，再代入分析，设 $z = x + yi(x,y\in \mathbf{R})$ ，
