...
<span class="fake-tag">解析</span>如图，由 $|AB|=10$ 结合对称性可知 $|AF_{2}|=5$ ，
...
<span class="fake-tag">解析</span>给出了一条渐近线，可通过比较渐近线斜率求 $m$
...
<span class="fake-tag">解析</span>所给方程表示焦点在 $x$ 轴上的双曲线，可将其化为标准方程 $\frac{x^2}{a^2} -\frac{y^2}{b^2} = 1$ 的形式，再比较系数，由 $\frac{x^2}{m + 1} +\frac{y^2}{m - 2} = 1$ 可得 $\frac{x^2}{m + 1} -\frac{y^2}{2 - m} = 1$
...
<span class="fake-tag">解析</span>由题意，双曲线的离心率 $e = \frac{c}{a} = \frac{\sqrt{a^2 + b^2}}{a} = \sqrt{5}$ ，
...
<span class="fake-tag">解析</span>先画图看双曲线的焦点到底是正方形对角的两个顶点，还是相邻的两个顶点，
...
<span class="fake-tag">解析</span>A项，当 $m > n > 0$ 时， $0 < \frac{1}{m} < \frac{1}{n}$ ，
...
<span class="fake-tag">解析</span>如图，直接分析 $|PA| - |PF_{1}|$ 的最大值不易，涉及
...
<span class="fake-tag">解析</span>双曲线的渐近线为 $y = \pm \sqrt{\frac{5}{m}} x$ ，由 $\sqrt{5} x + 2y = 0$ 可
...
记双曲线的右焦点为 $F'(3,0)$ ，则 $\left|PF\right| - \left|PF'\right| = 4$
...
$= \sqrt {\left| O C \right| ^ {2} + \left| O F ^ {'} \right| ^ {2}} = 5,$
