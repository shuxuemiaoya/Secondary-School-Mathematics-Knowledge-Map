...
<span class="fake-tag">解析</span>如图，由斜率可求得 $\cos \alpha$ ，于是选择角版焦半径公式来计算 $|AF|$ 和 $|BF|$
...
<span class="fake-tag">解析</span>: 我们知道 $\frac{1}{|AF|} + \frac{1}{|BF|} = \frac{2}{p}$ , 故可由此将 $|AF| + 4|BF|$ 凑成积为定值, 用均值不等式求最值,
...
<span class="fake-tag">解析</span>本题没有角度，故联立直线和抛物线，由韦达定理结合坐标版焦点弦公式算 $|AB|$ ，
...
<span class="fake-tag">解析</span>给了直线 l 的斜率，可求得其倾斜角，故考虑直接代公式 $S=\frac{p^{2}}{2\sin\alpha}$ 计算 $\triangle AOB$ 的面积，
...
<span class="fake-tag">解析</span>: 由 $\overrightarrow{AB} = 3\overrightarrow{FB}$ 可结合角版焦点弦、焦半径公式求角, 再用 $S = \frac{p^{2}}{2 \sin \alpha}$ 算 $\triangle AOB$ 的面积,
...
$\quad$A. $\left|AD\right| = \left|AF\right|$

$\quad$B. $|AE| = |AB|$

$\quad$C. $|AB|\geq 6$

$\quad$D. $|AE|\cdot|BE|\geq18$
