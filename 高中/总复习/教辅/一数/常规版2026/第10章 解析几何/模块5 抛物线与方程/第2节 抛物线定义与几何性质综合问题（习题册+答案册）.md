...
<span class="fake-tag">解析</span>如图，抛物线 $C$ 的焦点为 $F(\sqrt{3},0)$ ，准线为 $l$
...
<span class="fake-tag">解析</span>直线 $AB$ 的斜率即为图中 $\angle 5$ 的正切值，条件涉及角平分线，我们试试通过分析几何关系来求该角，
...
<span class="fake-tag">解析</span>给出 $3\overrightarrow{FM} = 2\overrightarrow{MN}$ ，可设 $FM$ 的长，并用它表示其它线段的长，抛物线的准线为 $x = -4$ ，焦点为 $F(4,0)$

如图，作 $MM' \perp$ 准线于 $M'$ ，交 $y$ 轴于点 $G$
...
$\left| M G \right| = \left| M M ^ {'} \right| - \left| M ^ {'} G \right| = 2 m - 4,$
...
<span class="fake-tag">解析</span>如图，已知 $|AF|$ ，只需求得 $|BF|$ 即可求出 $|AB|$ ，可先过A，B向准线作垂线，
...
$= 2\left|AA'\right|$ ，从而 $\cos \angle CAA' = \frac{|AA'|}{|AC|} = \frac{1}{2}$ ，故 $\angle CAA' = 60^{\circ}$
...
<span class="fake-tag">解析</span>条件给出 $|FA| = 2|FB|$ ，想到过 $A$ 和 $B$ 分别作准线的垂线，结合抛物线定义处理，
...
<span class="fake-tag">解析</span>如图，条件给出 $|BC|=2|BN|$ ，由抛物线定义还有 $|AM|=|AF|$ ， $|BN|=|BF|$ ，既然有这么多长度关系，那我们不妨设AF和BF的长，以此来分析图形的几何特征，
...
<span class="fake-tag">解析</span>抛物线的准线为 $x = -1 \Rightarrow p = 2 \Rightarrow$ 抛物线的方程为 $y^{2} = 4x$ ，其焦点为 $F(1,0)$ ， $A(-1,0)$
...
如图，作 $AA'\perp$ 准线于 $A'$ ， $BB'\perp$ 准线于 $B'$
...
<span class="fake-tag">解析</span>如图，以 $AB$ 为直径的圆过 $F \Rightarrow FA \perp FB$
...
$\left| P Q \right| = \frac {\left| A A ^ {'} \right| + \left| B B ^ {'} \right|}{2} = \frac {m + n}{2}, \quad \left| A B \right| = \sqrt {m ^ {2} + n ^ {2}},$
