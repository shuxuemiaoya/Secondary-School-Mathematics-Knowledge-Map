...
<span class="fake-tag">解析</span>两直线垂直，可用 $A_{1}A_{2} + B_{1}B_{2} = 0$ 翻译，
...
<span class="fake-tag">解析</span>涉及直线在两个坐标轴上的截距，常设直线的截距式方程，先考虑截距为0的特殊情况，
...
<span class="fake-tag">解析</span>斜率正负与倾斜角钝锐有关，故讨论 $\alpha$ 和 $\beta$ 的钝锐，
...
<span class="fake-tag">解析</span>要求倾斜角的范围，先求斜率的范围，
...
<span class="fake-tag">解析</span>直线 l 的方程含参，先观察是否过定点，
...
<span class="fake-tag">解析</span> $ax + y + 2a - 1 = 0\Rightarrow a(x + 2) + (y - 1) = 0$
...
<span class="fake-tag">解析</span>出现关于 $x, y$ 的一次分式结构，可考虑运用两点连线的斜率来分析，因为 $\frac{y}{x - 1} = \frac{y - 0}{x - 1}$ ，所以 $\frac{y}{x - 1}$ 可看成动点 $P(x,y)$ 与定点 $Q(1,0)$ 的连线斜率，
...
<span class="fake-tag">解析</span>如图， $OD$ 是正 $\triangle AOB$ 的一条高线，其斜率为3，
...
<span class="fake-tag">解析</span>只要找到外心、重心、垂心中的任意两个，就能求出欧拉线，再由欧拉线与 $l$ 平行求 $a$ ，我们画图来看哪两个心好找，如图， $\triangle ABC$ 为直角三角形，所以其外心为斜边中点 $D\left(0, \frac{3}{2}\right)$ ，垂心为直角顶点 $B(3,0)$ ，
