...
<span class="fake-tag">解析</span>已知两点，可先求斜率，再求倾斜角， $k=\frac{-1-2}{3-0}=-1\Rightarrow\tan\alpha=-1$ ，结合 $\alpha\in[0,\pi)$ 知 $\alpha=\frac{3\pi}{4}$ .
...
$\quad$B. $\left(\frac{\pi}{2},\frac{3\pi}{4}\right]$

$\quad$C. $\left[\frac{\pi}{2},\pi\right)$

$\quad$D. $\left[\frac{3\pi}{4},\pi\right)$

<span class="fake-tag">解析</span>已知两点坐标，可求出斜率的范围，再求倾斜角的范围，先考虑斜率不存在的情形，
...
$\quad$A. $\left(-\infty, -\frac{7}{2}\right] \cup \left[\frac{4}{3}, +\infty\right)$

$\quad$B. $\left[-\frac{7}{2}, \frac{4}{3}\right]$

$\quad$C. $\left(-\infty, -\frac{2}{7}\right] \cup \left[\frac{3}{4}, +\infty\right)$

$\quad$D. $\left[-\frac{7}{2}, \frac{3}{4}\right]$

<span class="fake-tag">解析</span>要求斜率的范围，不妨先画图分析直线 $l$ 的变动范围，找到临界状态，
...
$\quad$A. $\left(-\infty, -\frac{1}{2}\right] \cup [\sqrt{3}, +\infty)$
...
<span class="fake-tag">解析</span>出现关于 $x, y$ 的一次分式结构，可考虑用两点连线的斜率来分析，
...
<span class="fake-tag">解析</span>出现关于 $x, y$ 的一次分式结构，考虑运用斜率来分析，要凑出斜率，得先把分子的 $x$ 拆掉，因为 $\frac{x + y - 1}{x - 3} = \frac{(x - 3) + y + 2}{x - 3} = 1 + \frac{y + 2}{x - 3}$ ，记 $t = \frac{y + 2}{x - 3}$ ，则 $\frac{x + y - 1}{x - 3} = 1 + t$ ，
...
<span class="fake-tag">答案</span> $\left[-\frac{7}{5},\frac{3}{5}\right]$
...
<span class="fake-tag">解析</span>如图，设 $AD$ 是正 $\triangle ABC$ 的内角 $A$ 的平分线，则 $AD$ 与 $AB$ 和 $AC$ 的夹角均为 $30^{\circ}$ ，
...
<span class="fake-tag">答案</span> $\frac{\sqrt{3}}{5}$ （或 $3\sqrt{3}$ ）
...
<span class="fake-tag">解析</span>已知 $\sin \alpha$ 可求出 $\tan \alpha$ ，得到斜率，用点斜式写出直线的方程，
...
<span class="fake-tag">答案</span> $y = 2x - 1$ 或 $y = -2x + 3$

【例 5】已知△ABC 中，已知 B(2,1)，C(-2,3)，则边 BC 的中垂线 l 的方程为 \_\_\_\_.

<span class="fake-tag">解析</span>如图，中垂线 $l$ 过 $BC$ 中点 $D(0,2)$ ，还差斜率，可先求 $BC$ 的斜率，再由 $l \perp BC$ 求 $l$ 的斜率，

由题意， $k_{BC} = \frac{3 - 1}{-2 - 2} = -\frac{1}{2}$ ，所以 $-\frac{1}{2} k_l = -1$ ，从而 $k_{l} = 2$

故 $l$ 的方程为 $y - 2 = 2(x - 0)$ ，即 $2x - y + 2 = 0$ 。

<span class="fake-tag">答案</span> $2x - y + 2 = 0$
...
<span class="fake-tag">解析</span>涉及截距，可设直线的截距式方程，该方程不能表示截距为0的情形，故先考虑这种特殊情况，当直线 $l$ 过原点时，其斜率为2，故其方程为 $y = 2x$ ，即 $2x - y = 0$
...
<span class="fake-tag">答案</span> $2x - y = 0$ 或 $x + y - 3 = 0$
...
<span class="fake-tag">解析</span>涉及 $l_{1} / / l_{2}$ ，可先用 $A_{1}B_{2} = A_{2}B_{1}$ 求出参数 $a$ 的值并检验是否重合，得到 $l_{1} / / l_{2}$ 的充要条件再看， $l_{1} / / l_{2} \Rightarrow (a + 1)a = 2a^{2} \Rightarrow a = 0$ 或1，经检验， $a = 1$ 时 $l_{1}$ 与 $l_{2}$ 重合，故“ $a = 0$ ”是“ $l_{1} / / l_{2}$ ”的充要条件.
...
<span class="fake-tag">解析</span>涉及 $l_{1} \perp l_{2}$ ，可用 $A_{1}A_{2} + B_{1}B_{2} = 0$ 求参数的值，由题意， $l_{1} \perp l_{2} \Leftrightarrow (a + 2)(a - 1) + (1 - a)(2a + 3) = 0$ ，解得： $a = \pm 1$ ，所以“ $a = 1$ ”是“ $l_{1} \perp l_{2}$ ”的充分不必要条件.

<span class="fake-tag">答案</span>: A

##### 【变式】若直线 $l_{1}: x - my + 1 = 0$ 过定点 $A, l_{2}: mx + y - m + 3 = 0$ 过定点 $B, l_{1}$ 与 $l_{2}$ 交于点 $P$ ，则 $\left|PA\right|^2 + \left|PB\right|^2 =$ \_\_\_\_.

<span class="fake-tag">解析</span>为了找到两直线所过的定点，常先把参数集中起来再看，
...
$\quad$A. $2x + y - 12 = 0$   
$\quad$B. $2x + y - 12 = 0$ 或 $2x - 5y = 0$   
$\quad$C. $x - 2y - 1 = 0$   
$\quad$D. $x + 2y - 9 = 0$ 或 $2x - 5y = 0$
...
$\quad$A. $\left[0, \frac{\pi}{4}\right) \cup \left(\frac{3\pi}{4}, \pi\right)$

$\quad$B. $\left(-\frac{\pi}{4},\frac{\pi}{4}\right)$

$\quad$C. $\left[0, \frac{\pi}{4}\right] \cup \left[\frac{3\pi}{4}, \pi\right]$

$\quad$D. $\left[-\frac{\pi}{4}, \frac{\pi}{4}\right]$
...
$\quad$B. $\left[\frac{\pi}{4},\frac{3\pi}{4}\right]$

$\quad$C. $\left[0, \frac{\pi}{6}\right] \cup \left[\frac{5\pi}{6}, \pi\right)$
...
$\quad$A. $\frac{y}{x - 1}$ 的最大值为 $\sqrt{3}$

$\quad$B. $\frac{y}{x - 1}$ 的最小值为 $-\sqrt{3}$

$\quad$C. $\frac{y}{x - 1}$ 的最大值为 $\frac{\sqrt{3}}{3}$

$\quad$D. $\frac{y}{x - 1}$ 的最小值为 $-\frac{\sqrt{3}}{3}$
