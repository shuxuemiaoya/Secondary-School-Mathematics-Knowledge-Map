...
解析：直接分析 $f(x)$ 的零点不易，考虑将 $f(x) = 0$ 变形，便于作图分析交点个数， $f(x) = 0 \Leftrightarrow \left(\frac{1}{2}\right)^x = \cos x$
...
解析：由题意， $f(x) = \cos \left[2\left(x + \frac{\pi}{6}\right) + \frac{\pi}{6}\right] = \cos \left(2x + \frac{\pi}{2}\right)$
