...
<span class="fake-tag">解析</span> $g(x)=0\Leftrightarrow f(x)=b$ ，所以问题等价于存在水平直线y=b和 $f(x)$ 的图象有3个交点，
...
<span class="fake-tag">解析</span> $g(x)=0\Leftrightarrow f(x)=b$ ，故 $g(x)$ 有3个零点 $\Leftrightarrow$ 直线y=b与 $f(x)$ 的图象有3个交点，
...
<span class="fake-tag">解析</span>观察发现 $f(x)$ 两段的解析式都不复杂，故可考虑画出两段解析式完整的图象，再看分段点在何处能满足题设条件， $y = x^3 - 3x \Rightarrow y' = 3x^2 - 3 = 3(x + 1)(x - 1)$ ，
...
<span class="fake-tag">解析</span>当 $a = 1$ 时， $f(x) = \begin{cases} \cos \pi x, & 0 < x < 1 \\ x^2 - 4x + 8, & x \geq 1 \end{cases}$ ，
