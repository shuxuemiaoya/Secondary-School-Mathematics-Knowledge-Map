...
解析：条件中有两条对称轴，以及它们之间的单调性，由此容易画出大致图象，故考虑画图来分析，
...
解析：图上标注了零点-1和最大值点1，可由此求出周期，进而求得 $\omega$ ，由图可知， $1 - (-1) = \frac{T}{4}$ ，所以 $T = 8$
...
解析：观察发现 $x_{1}$ ， $x_{2}$ 是 $f(x)$ 的零点，所以可先求出它们，用于计算 $\frac{x_{2}}{x_{1}}$ ，由 $f(x) = 0$ 可得 $\sin (\omega x + \varphi) = 0$ ，
...
解析：要求最小正周期，就是要求 $\omega$ ，怎么构造方程？图上只有 $\left(-\frac{4\pi}{9},0\right)$ 这一个点可代入解析式，故把它代进去，
