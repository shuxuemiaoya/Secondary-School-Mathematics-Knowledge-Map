...
B. 若 $\triangle ABC$ 的直观图是边长为 2 的等边三角形, 那么 $\triangle ABC$ 的面积为 $\sqrt{6}$

C. $\triangle ABC$ 的直观图如图所示, $A'B'$ 在 $x'$ 轴上, $A'B' = 2$ , $B'C'$ 与 $x'$ 轴垂直,

且 $B^{\prime}C^{\prime} = \sqrt{2}$ ，则 $\triangle ABC$ 的面积为4
...
解析：A 项，斜二测画法不改变平行关系，也不改变平行线段的长度大小关系，所以梯形的直观图仍旧是梯形，故 A 项正确；
...
解析：如图，以 $A$ 为顶点求体积，高不好找，但若转换成以 $D_{1}$ 为顶点，则高即为 $A_{1}D_{1}$ ，底面积也好算，
...
解析：如图，直接算距离需作垂线，较麻烦，但观察发现 $V_{C - PAB}$ 和 $S_{\triangle ABC}$ 好求，故用等体积法， $\left\{ \begin{array}{l}AP = BP\\ AB\text{为直径} \end{array} \right.\Rightarrow \triangle PAB$ 是等腰 Rt 三角形，又 $AB = 8$ ，所以 $PA = PB = 4\sqrt{2}$ ， $S_{\triangle PAB} = \frac{1}{2}\times (4\sqrt{2})^2 = 16$
...
解析：直线 $MN$ 和点 $A$ 分别在上、下两个平行的面内，但若过 $A$ 作 $MN$ 的平行线 $l$ ，则 $l$ 在正方体外，如图1，不像例4那么好处理了，怎么办呢？此时可用“延长线法”，我们一般通过延长表面的线，找它与棱的交点，从而扩大截面。可以发现 $\triangle AMN$ 中的 $MN$ 在表面，所以延长它，
