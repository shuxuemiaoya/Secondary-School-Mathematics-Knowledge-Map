...
$\quad$A. $\overrightarrow{AB}$ 与 $\overrightarrow{CD}$ 不共面

$\quad$B. $\overrightarrow{AB} +\overrightarrow{BC} -\overrightarrow{GC} -\overrightarrow{EG} = \overrightarrow{EB}$

$\quad$C. $\overrightarrow{AB} \cdot \overrightarrow{AC} = 2$

$\quad$D. $\overrightarrow{EF} \perp \overrightarrow{FG}$

<span class="fake-tag">解析</span>A项，向量可以平移，空间中任意两个向量都可平移到同一平面上去，故A项错误；
...
$\quad$B. $5|\pmb {a}| = \sqrt{3} |\pmb {b}|$

$\quad$C. $a \perp (5a + 4b)$

$\quad$D. $a$ 在 $b$ 上的投影向量为 $-\frac{1}{10} b$

<span class="fake-tag">解析</span>A项，由题意， $2a+b=(-4,-2,2)+(3,4,5)=(-1,2,7)$ ，观察发现不存在实数 $\lambda$ ，使得 $2a+b=\lambda a$ ，所以 $2a+b$ 与a不平行，故A项错误；
...
<span class="fake-tag">解析</span>不能构成基底，说明共面．可发现 m 与 n 不共线，故 p 与 m，n 共面等价于 p 能用 m 和 n 表示，设 $p = \lambda m + \mu n$ ，则 $3a + xb + c = \lambda (a - 2b) + \mu (a + b + c)$ ，整理得： $3a + xb + c = (\lambda + \mu)a + (\mu - 2\lambda)b + \mu c$ ，
...
$\quad$A. $A_{1}C \perp AB_{1}$

$\quad$B. $A_{1} B$ 与 $A D_{1}$ 所成的角为 $60^{\circ}$

$\quad$C. $DD_{1} \perp AF$

$\quad$D. 平面 $AEF$ 的一个法向量为 $\pmb{m} = (4,2,4)$

<span class="fake-tag">解析</span>A项，判断两直线是否垂直，只需看它们的方向向量数量积是否为0，
...
$\quad$A. $\frac{1}{2} (\pmb {c} - \pmb {a} - \pmb{b})$

$\quad$B. $\frac{1}{2} (\pmb {c} + \pmb {a} + \pmb{b})$

$\quad$C. $\frac{1}{2} (\pmb {a} + \pmb {b} - \pmb{c})$

$\quad$D. $-\frac{1}{2} (\pmb {c} + \pmb {a} - \pmb{b})$
...
$\quad$B. 与 $\overrightarrow{AB}$ 同向的单位向量是 $\left(\frac{2\sqrt{5}}{5}, \frac{\sqrt{5}}{5}, 0\right)$

$\quad$C. $\overrightarrow{AB}$ 和 $\overrightarrow{BC}$ 的夹角余弦值是 $\frac{\sqrt{55}}{11}$

$\quad$D. 平面 $ABC$ 的一个法向量是 $(1, -2, 5)$
