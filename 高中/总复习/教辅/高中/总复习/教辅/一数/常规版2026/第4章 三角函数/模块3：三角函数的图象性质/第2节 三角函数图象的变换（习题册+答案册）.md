...
解析：先将系数化1， $y = \cos \left(2x - \frac{\pi}{4}\right) = \cos 2\left(x - \frac{\pi}{8}\right)$ ，
...
解析：先化同名，二者 $x$ 的系数相同，可利用 $\cos \alpha =$
...
解析：函数名相同， $x$ 的系数相反，得先将 $x$ 的系数化为相同，可用 $\sin \alpha = \sin (\pi -\alpha)$ 来化，
...
解析：将 $y = \sin \left(2x + \frac{\pi}{3}\right)$ 左移 $\frac{\pi}{6}$ ，得到 $y =$
...
解析：由题意， $g(x) = f\left(x + \frac{\pi}{6}\right) = \sin \left[2\left(x + \frac{\pi}{6}\right) + \varphi \right]$
...
解析：A 项，求 $\varphi$ 考虑代点，代谁呢？观察图形可发现， $A$ 容易由最大值直接得出，那么将 $(0, -1)$ 代入解析式，就能得到只含 $\varphi$ 的方程，从而求出 $\varphi$ ，
