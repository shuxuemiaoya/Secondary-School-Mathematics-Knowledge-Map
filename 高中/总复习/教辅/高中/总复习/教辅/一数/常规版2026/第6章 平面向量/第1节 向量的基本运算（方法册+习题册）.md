...
<span class="fake-tag">解析</span>方向相反属于共线的情形，故可考虑用共线向量定理处理，
...
<span class="fake-tag">解析</span> $A, B, D$ 三点共线可用向量共线翻译，即存在 $\lambda$ 使 $\overrightarrow{AD} = \lambda \overrightarrow{AB}$ ，故将 $\overrightarrow{AD}$ 和 $\overrightarrow{AB}$ 用 $\vec{e}_1, \vec{e}_2$ 表示，
...
<span class="fake-tag">解析</span>看到模，先试试平方，由题意， $|a| = 1$ ， $|b| = 1$ ， $|a - b| = 1$ ，所以 $\left|\boldsymbol {a} - \boldsymbol {b}\right|^2 = \boldsymbol {a}^2 +\boldsymbol {b}^2 -2\boldsymbol {a}\cdot \boldsymbol {b} = 2 - 2\boldsymbol {a}\cdot \boldsymbol {b} = 1$ 从而 $\pmb {a}\cdot \pmb {b} = \frac{1}{2}$ ，故 $\left|\boldsymbol {a} - 3\boldsymbol {b}\right|^2 = \boldsymbol {a}^2 +9\boldsymbol {b}^2 -6\boldsymbol {a}\cdot \boldsymbol {b} = 10 - 6\times \frac{1}{2} = 7$ ，所以 $|\boldsymbol {a} - 3\boldsymbol {b}| = \sqrt{7}$
...
<span class="fake-tag">解析</span>给了模的不等式，先试试平方去掉模，看能得到什么，设 $a$ 与 $b$ 的夹角为 $\theta$
...
<span class="fake-tag">答案</span> $\frac{\pi}{3}$ ； $\frac{\sqrt{3}}{2}$
...
<span class="fake-tag">解析</span>先看充分性，可将两个数量积用定义表示出来再分析，
...
<span class="fake-tag">解析</span> $c\perp d$ 可用数量积翻译，由题意， $\pmb {c}\cdot \pmb {d} = (2\pmb {a} - kb)\cdot (\pmb {a} + kb) = 2\pmb{a}^2 +2ka\cdot \pmb {b} - ka\cdot \pmb {b} - k^2\pmb{b}^2 = 0$ ①
...
<span class="fake-tag">答案</span> $\frac{1\pm\sqrt{33}}{2}$
...
<span class="fake-tag">解析</span>向量问题中涉及夹角，一般考虑夹角余弦公式，先算它们的数量积，看看还差什么，
...
$\quad$A. 若 $a \parallel b$ ，则存在唯一实数 $\lambda$ ，使 $a = \lambda b$

$\quad$B. 若向量 $a, b$ 所在的直线是异面直线，则 $a, b$ 一定不共面

$\quad$C. 若 $a$ 是非零向量，则 $\frac{a}{|a|}$ 是与 $a$ 同向的单位向量

$\quad$D. 若 $a, b$ 都是非零向量，则 $\frac{a}{|a|} + \frac{b}{|b|} = 0$ ”是“ $a$ 与 $b$ 共线”的充分不必要条件
...
$\quad$A. $\left\| \boldsymbol {a}\right| - \left|\boldsymbol {b}\right|\leq \left|\boldsymbol {a} + \boldsymbol {b}\right|\leq \left|\boldsymbol {a}\right| + \left|\boldsymbol {b}\right|$   
$\quad$B. 若 $a, b$ 为非零向量，且 $\vert a + b \vert = \vert a - b\|$ ，则 $a \perp b$   
$\quad$C. $|\pmb{a}| - |\pmb{b}| = |\pmb{a} + \pmb{b}|$ 是 $\pmb{a}$ , $\pmb{b}$ 共线的充要条件  
$\quad$D. 若 $a, b$ 为非零向量，且 $\|a| - |b| = |a - b|$ ，则 $a$ 与 $b$ 同向
...
$\quad$B. $\vec{e} \perp (\vec{a} - \vec{e})$

$\quad$C. $\vec{a} \perp (\vec{a} - \vec{e})$
