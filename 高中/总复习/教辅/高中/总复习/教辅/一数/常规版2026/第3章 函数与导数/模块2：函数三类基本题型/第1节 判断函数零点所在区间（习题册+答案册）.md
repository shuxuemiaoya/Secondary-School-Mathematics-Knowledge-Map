...
<span class="fake-tag">解析</span>因为 $y = 0.3^{x}$ 和 $y = -\sqrt{x}$ 都在 $[0, +\infty)$ 上 $\searrow$
...
<span class="fake-tag">解析</span>因为 $y = 2^{x}$ 和 $y = 3x + a$ 都在 $\mathbf{R}$ 上 $\nearrow$
...
<span class="fake-tag">解析</span>先判断单调性，这里需要求导来判断，
...
<span class="fake-tag">解析</span>由题意， $f'(x) = \frac{1}{3} -\frac{1}{x} = \frac{x - 3}{3x}$

所以 $f'(x) > 0\Leftrightarrow x > 3$ ， $f'(x) <   0\Leftrightarrow 0 <   x <   3$
...
<span class="fake-tag">解析</span>因为 $y = \mathrm{e}^{-x}$ 和 $y = -2x - 5$ 都在 $\mathbf{R}$ 上 $\searrow$
