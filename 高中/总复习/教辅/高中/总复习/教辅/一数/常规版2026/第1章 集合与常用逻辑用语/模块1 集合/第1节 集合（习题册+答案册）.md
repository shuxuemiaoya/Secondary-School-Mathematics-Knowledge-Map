...
<span class="fake-tag">解析</span> $x^{3} = x\Leftrightarrow x^{3} - x = x(x^{2} - 1) = x(x + 1)(x - 1) = 0$
...
<span class="fake-tag">解析</span>因为 $4 \in A$ ，所以 $a^2 - 2a + 1 = 4$ 或 $a - 4 = 4$
...
<span class="fake-tag">解析</span>观察发现两个集合中都有元素 $a$ ，故就看 1， $b$ 怎样与 $a^2$ ， $ab$ 对应了，有两种可能的情况，故讨论，
...
<span class="fake-tag">解析</span>由 $|x - 3|\leq m$ 可得 $-m\leq x - 3\leq m$
...
$\quad$C. $(\complement_{U}A)\cap B = \{4,5,6\}$   
$\quad$D. $A \cap B$ 的真子集个数是 3

##### 5. BCD

<span class="fake-tag">解析</span>观察发现 $B$ 中的元素用 $a + b$ 代表，而 $a, b$ 又来自集合 $A$ ，故可将 $a + b$ 可能的值直接全部罗列出来，再分析，因为 $a, b \in A$ ，所以 $a, b$ 可能的组合如下表：
...
<span class="fake-tag">解析</span>设 $A \cap B$ 中有 $n$ 个元素，则 $A \cap B$ 有 $2^{n}$ 个子集，
...
<span class="fake-tag">解析</span>由 $-5 < x^{3} < 5$ 可得 $-\sqrt[3]{5} < x < \sqrt[3]{5}$
...
<span class="fake-tag">解析</span> $|x - 1|\leq 1\Leftrightarrow -1\leq x - 1\leq 1\Leftrightarrow 0\leq x\leq 2$ ，故 $A = [0,2]$
...
<span class="fake-tag">解析</span>由题意，要使 $\sqrt{x} \in A$ ，则 $\sqrt{x} = 1,2,3,4,5,9$
...
<span class="fake-tag">解析</span> $x^{2} - 1\geq 0\Rightarrow x^{2}\geq 1\Rightarrow x\leq -1$ 或 $x\geq 1$
...
<span class="fake-tag">解析</span> $\frac{4 - x}{x + 1}\geq 0\Leftrightarrow \left\{ \begin{array}{l}(4 - x)(x + 1)\geq 0\\ x + 1\neq 0 \end{array} \right.$ ，解得： $-1 <   x\leq 4$
...
<span class="fake-tag">解析</span> $x(x - 2) <   0\Leftrightarrow 0 <   x <   2$ ，所以 $M = \{x\mid 0 <   x <   2\}$
...
<span class="fake-tag">解析</span>涉及三个小组，且彼此的人员有重叠，关系较复杂，可考虑画图分析，
...
<span class="fake-tag">解析</span>观察发现集合 $A, B$ 都是点集，故它们所代表的图形的交点个数即为 $A \cap B$ 中元素的个数，
...
<span class="fake-tag">解析</span>A项，由题意， $1\in \mathbf{N}$ ， $2\in \mathbf{N}$ ，但 $1 - 2 = -1\notin \mathbf{N}$
