...
<span class="fake-tag">解析</span>向量垂直可用数量积为 0 来翻译，此处可以先求两个向量的坐标，再算数量积，但若注意到 $a \cdot b = 0$ ，则会发现直接展开计算量更小，
...
$\quad$A. $\left(\frac{5\sqrt{3}}{8}, \frac{5}{8}\right)$

$\quad$B. $\left(\frac{5\sqrt{3}}{4},\frac{5}{4}\right)$

$\quad$C. $\left(\frac{\sqrt{3}}{8},\frac{1}{8}\right)$

$\quad$D. $\left(\frac{\sqrt{3}}{4},\frac{1}{4}\right)$

##### 3. A

<span class="fake-tag">解析</span>算投影向量，可代内容提要1中的公式⑦，其核心是算 $a \cdot b$ ，观察发现把 $(2a - b) \cdot b = 3$ 展开，就能产生 $a \cdot b$ ，
...
<span class="fake-tag">解析</span>由所给信息可发现，涉及的是向量的加法、减法，且有关向量放到了坐标系下，故考虑用坐标运算处理，为了便于分析，下面先设出有关向量，
...
$\quad$B. $\left(\frac{\sqrt{3}}{3},+\infty\right)$
...
$\quad$A. $\frac{1}{3} \leq x \leq 1$   
$\quad$B. $\overrightarrow{BD} = \frac{1}{3}\overrightarrow{BA} +\frac{2}{3}\overrightarrow{BC}$   
$\quad$C. $\frac{9}{2} \leq \overrightarrow{BP} \cdot \overrightarrow{BC} \leq 9$

$\quad$D. $x + y$ 的最大值为 $\frac{2\sqrt{3}}{9} + 1$

![](images/c4764528047fdb0349f8b689f6d70378d57e3d1c2116eefdd47abfcd349d0ee5.jpg)

##### 7. BCD

<span class="fake-tag">解析</span>涉及圆上动点，可考虑建系，利用圆的方程把动点坐标设为三角形式，结合图形可发现也容易建系，故按此处理，以点 $B$ 为原点建立如图所示的平面直角坐标系，
