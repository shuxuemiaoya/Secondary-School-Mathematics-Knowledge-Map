...
解析：看到过原点的直线与双曲线交于 $A, B$ 两点，想到和两焦点构成平行四边形，
...
解析：条件涉及 $PF_{2}$ 的斜率，怎样翻译？有 $\triangle PF_1F_2$ 为 Rt $\triangle$ ，当然到此三角形中研究边的关系，设直线 $PF_{2}$ 的倾斜角为 $\alpha$ ，则 $\angle PF_2F_1 = \alpha$ ，由题意， $k_{PF_2} = \tan \alpha = 2$ ，
...
解析：如图，条件中有 $\overrightarrow{F_2A} = -\frac{2}{3}\overrightarrow{F_2B}$ ，不妨设一段长度，看能否表示其余线段的长，
...
解析：如图，由双曲线的对称性可知 $A$ ， $B$ 关于原点对称，即 $O$ 是线段 $AB$ 的中点，
