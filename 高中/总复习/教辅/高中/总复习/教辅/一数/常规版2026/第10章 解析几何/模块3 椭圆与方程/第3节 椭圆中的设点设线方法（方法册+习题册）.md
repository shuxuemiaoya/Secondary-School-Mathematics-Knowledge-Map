...
<span class="fake-tag">解析</span> $\overrightarrow{F_1P}\cdot \overrightarrow{F_2P}$ 可用坐标计算，故考虑设点 $P$ 的坐标，设 $P(x,y)$ ，由题意， $F_{1}(-1,0)$ ， $F_{2}(1,0)$
...
设图2中 $l':x + y + m = 0$ ，联立 $\left\{ \begin{array}{l}x + y + m = 0\\ \frac{x^2}{16} +\frac{y^2}{9} = 1 \end{array} \right.$ 消去 $y$ 整理得： $25x^{2} + 32mx + 16m^{2} - 144 = 0$
...
<span class="fake-tag">解析</span>如图，用几何的方法不易翻译已知条件，分析发现点 $M$ 在线段 $PF$ 上运动，只要设 $M$ 的坐标，就能表示 $E, T$ 的坐标，进而利用 $B, T, M$ 三点共线建立方程求离心率，
...
<span class="fake-tag">答案</span> $\left(-\sqrt{2}, - \frac{\sqrt{2}}{2}\right)\cup \left(\frac{\sqrt{2}}{2},\sqrt{2}\right)$
...
$\quad$B. $\frac{x^2}{6} +\frac{y^2}{5} = 1$

$\quad$C. $\frac{x^2}{9} +\frac{y^2}{8} = 1$

$\quad$D. $\frac{x^2}{36} +\frac{y^2}{32} = 1$
