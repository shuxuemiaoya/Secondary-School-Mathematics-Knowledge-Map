...
<span class="fake-tag">解析</span>空间基底表示和平面基底表示方法类似，往与基向量关联较强的向量上化即可，
...
<span class="fake-tag">解析</span>由题意， $a\cdot b = 2\times 1 + (-1)\times (-2) + 2\times 1 = 6$
...
<span class="fake-tag">解析</span>注意到 $a, b$ 不共线，所以 $a, b, c$ 共面等价于 $c$ 能用 $a, b$ 表示，可由此建立方程组求 $x$
...
<span class="fake-tag">解析</span>要判断三个向量是否构成基底，就看它们是否不共面。观察发现选项中 $a + b$ 和 $a - b$ 不共线，故通过判断能否用它们表示另一向量来看它们是否共面，
...
<span class="fake-tag">解析</span>A项， $\overrightarrow{AB} = (2,1,0)$ ， $\overrightarrow{AC} = (-1,2,1)$ ， $\overrightarrow{AB}\cdot \overrightarrow{AC} =$
