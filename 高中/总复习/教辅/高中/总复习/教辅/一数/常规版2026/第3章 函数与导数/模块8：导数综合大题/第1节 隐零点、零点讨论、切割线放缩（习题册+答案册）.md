...
由题意， $f^{\prime}(x) = \mathrm{e}^{x} - a$ ，当 $a\leq 0$ 时， $f^{\prime}(x) > 0$ 恒成立，

所以 $f(x)$ 在 $\mathbf{R}$ 上单调递增， $f(x)$ 最多1个零点，不合题意；

当 $a > 0$ 时，由 $f^{\prime}(x) > 0$ 得 $x > \ln a$ ，由 $f^{\prime}(x) < 0$ 得 $x < \ln a$
...
因为 $x > 0$ ，所以 $f^{\prime}(x) = \frac{2ax^{2} - 1}{x} < 0$
...
令 $F(x) = x\mathrm{e}^{x} - \ln x - x - 1$ ， $x > 0$ ，则 $F^{\prime}(x) =$

$$
\mathrm{e} ^ {x} + x \mathrm{e} ^ {x} - \frac {1}{x} - 1 = (1 + x) \mathrm{e} ^ {x} - \frac {x + 1}{x} = (x + 1) \left(\mathrm{e} ^ {x} - \frac {1}{x}\right),
$$

（显然 $x + 1 > 0$ ，故 $F^{\prime}(x)$ 的正负由 $\mathrm{e}^x -\frac{1}{x}$ 决定，这部分的正负情况不易直接看出，考虑单独拿出来求导分析）
...
由题意， $f^{\prime}(x) = \mathrm{e}^{2x} - a + (x + b)\cdot 2\mathrm{e}^{2x} = (2x + 2b + 1)\mathrm{e}^{2x} - a$
...
因为 $F(x) = f(x) - h(x)$ ，所以 $F^{\prime}(x) = f^{\prime}(x) - h^{\prime}(x)$
...
$= x\mathrm{e}^{x} - 2ax = x(\mathrm{e}^{x} - 2a)$ ，（由 $f^{\prime}(x) = 0$ 可得 $x = 0$ 或 $\ln (2a)$ ，其中 $\ln (2a)$ 是否有意义由 $a$ 的正负决定，故据此讨论）
...
所以 $f^{\prime}(x) > 0$ ，故 $f(x)$ 在 $(-\infty ,\ln (2a))$ 上单调递增，

在 $(\ln (2a),0)$ 上单调递减，在 $(0, + \infty)$ 上单调递增；

当 $a = \frac{1}{2}$ 时， $f'(x) = x(\mathrm{e}^x - 1)$ ，若 $x < 0$ ，则 $\mathrm{e}^x - 1 < 0$

所以 $f^{\prime}(x) > 0$ ，若 $x > 0$ ，则 $\mathrm{e}^x -1 > 0$ ，所以 $f^{\prime}(x) > 0$
