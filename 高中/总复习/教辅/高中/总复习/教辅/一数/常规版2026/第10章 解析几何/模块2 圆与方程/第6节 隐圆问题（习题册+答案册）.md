...
<span class="fake-tag">解析</span> $PA \perp PB$ 意味着点 P 在以 AB 为直径的圆上，所以题设条件等价于圆 C 上有且仅有一点在以 AB 为直径的圆上，这可按两圆的位置关系问题处理，且有外切和内切两种情况，下面分别考虑，
...
<span class="fake-tag">解析</span>圆 $C$ 的方程可化为 $(x - 3)^2 +(y - 3)^2 = 18 + m$
...
<span class="fake-tag">解析</span> $A$ ， $B$ 是定点，所以由 $\left|PB\right| = 2\left|PA\right|$ 可约束 $P$ 的轨迹，故先求出轨迹方程，设 $P(x,y)$ ，因为 $\left|PB\right| = 2\left|PA\right|$ ，所以 $\sqrt{(x - 4)^2 + y^2} = 2\sqrt{(x - 1)^2 + y^2}$ ，整理得： $x^{2} + y^{2} = 4$ ，
...
<span class="fake-tag">解析</span>直线 l 含参，先看它是否过定点，
...
<span class="fake-tag">解析</span>钝角这种条件怎么翻译？我们知道直角可翻译成“圆上”，那钝角呢？翻译成“圆内”即可，
