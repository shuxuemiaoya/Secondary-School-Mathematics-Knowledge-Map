...
<span class="fake-tag">解析</span>涉及焦点三角形，考虑用双曲线的定义处理，如图，设 $\left|PF_{1}\right|=m,\left|PF_{2}\right|=n$ ，则 $\left|m-n\right|=2a$ ①，条件 $PF_{1}\perp PF_{2}$ 怎样翻译？由于上面得到的是长度关系，所以考虑用勾股定理来翻译，
...
<span class="fake-tag">解析</span>先分析图形，在双曲线中给出右焦点，一定要关注左焦点，
...
<span class="fake-tag">解析</span>如图，因为 $\left|F_{1}P\right|=2\left|F_{1}H\right|$ ，所以H为 $F_{1}P$ 的中点，涉及中点，想到中位线，又因为原点O为 $F_{1}F_{2}$ 的中点，所以 $\left|PF_{2}\right|=2\left|OH\right|=2a$ ，且 $PF_{2}//OH$ ，
...
<span class="fake-tag">解析</span>涉及双曲线的左焦点 $F$ ，我们把右焦点 $F'$ 也取出来，条件中有中点，想到构造中位线，
...
<span class="fake-tag">解析</span>涉及 $\left|PF_{1}\right|$ 和 $\left|PF_{2}\right|$ ，考虑双曲线定义，由题意， $\left\{\begin{aligned}&\left|PF_{1}\right|=3\left|PF_{2}\right|\\&\left|PF_{1}\right|-\left|PF_{2}\right|=2a\end{aligned}\right.$ ，所以 $\left\{\begin{aligned}&\left|PF_{1}\right|=3a\\&\left|PF_{2}\right|=a\end{aligned}\right.$
...
<span class="fake-tag">解析</span>如图，涉及双曲线上的点和两个焦点，考虑定义，由题意， $\left\{ \begin{array}{l} \left|AF_1\right| - \left|AF_2\right| = 2a \\ \left|BF_1\right| - \left|BF_2\right| = 2a \end{array} \right.$ ①
...
<span class="fake-tag">解析</span>如图，涉及双曲线上的点和两个焦点，考虑结合定义求有关线段的长，
...
<span class="fake-tag">解析</span>如图，给出 $|AB| = \sqrt{2}|F_1P|$ ，可由此设出 $|AB|$ ，并尝试求其它线段的长，
...
<span class="fake-tag">答案</span> $1 + \sqrt{3}$
...
$\quad$A. 射线 $n$ 所在直线的斜率 $k \in \left(-\frac{4}{3}, \frac{4}{3}\right)$

$\quad$B. 当 $m \perp n$ 时， $\left|PF_{1}\right| \cdot \left|PF_{2}\right| = 32$

$\quad$C. 当射线 $n$ 过点 $Q(7,5)$ 时, 光线由 $F_{2}$ 到 $P$ , 再到 $Q$ 经过的路程为 13

$\quad$D. 若 $T(1,0)$ , 直线 $PT$ 与 $C$ 相切, 则 $\left|PF_{2} \right| = 12$
...
<span class="fake-tag">解析</span>A项，如下图1，射线 $n$ 的斜率即为直线 $PF_{1}$ 的斜率，当 $P$ 在双曲线右支上运动时， $PF_{1}$ 可从 $l_{2}$ 绕点 $F_{1}$ 逆时针旋转至 $l_{1}$ （临界状态均不可取）， $l_{1}$ 、 $l_{2}$ 分别与渐近线平行，
