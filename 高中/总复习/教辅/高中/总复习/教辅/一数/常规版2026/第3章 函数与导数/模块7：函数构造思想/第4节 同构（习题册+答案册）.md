...
<span class="fake-tag">解析</span>所给不等式中 $x$ 和 $y$ 的部分结构类似，先将它们分离到不等号两侧，看能否化为同构形式，
...
设 $f(x) = x - \cos x(x\in \mathbf{R})$ ，则 $f'(x) = 1 + \sin x\geq 0$
...
<span class="fake-tag">解析</span>观察发现所给不等式中 $x_{1}$ 和 $x_{2}$ 的部分结构类似，先将它们分离到不等号的两侧，看能否化同构形式，
...
从而 $f'(x) = 1 - \frac{a}{3x}\leq 0$ 在(1,3]上恒成立，故 $a\geq 3x$
...
<span class="fake-tag">解析</span>已知的两个方程都无法解出 $a$ 和 $b$ ，尝试同构，
...
<span class="fake-tag">解析</span>由题意， $h(x)\geq 0\Leftrightarrow x\mathrm{e}^{x + 1} - k(\ln x + x + 1)\geq 0$
...
<span class="fake-tag">解析</span> $x^{x}$ 的底数和指数都有 x，不易直接分析，怎么办呢？可通过取对数把指数部分的 x 拿下来，
...
<span class="fake-tag">解析</span>左边是 $\ln (mx + 1)$ ，所以将右侧的 $mx$ 移至左侧，凑
...
<span class="fake-tag">解析</span>由已知的等式无法求出 $m$ ，于是尝试同构，等式中
...
<span class="fake-tag">解析</span>原不等式 $\Leftrightarrow a\mathrm{e}^{a + 1} < b(\ln b - 1) \Leftrightarrow a\mathrm{e}^{a + 1} < b\ln \frac{b}{\mathrm{e}}$
...
<span class="fake-tag">解析</span>指、对的底数是2，不是e，能同构吗？能，其实 $x\mathrm{e}^{x}$ 和 $x\ln x$ 的同构方法，也适用于其它底数的情形，所以我们先移项把指、对分开，再观察 $\log_{2}x$ 和 $m\cdot2^{mx}$ 这两个部分，可发现同乘以x就能转化为 $x\log_{2}x$ 和 $mx\cdot2^{mx}$ ，
...
$f(mx)$ ，且 $f'(x) = 2^{x} + x\cdot 2^{x}\cdot \ln 2 = 2^{x}(1 + x\cdot \ln 2)$

所以 $f'(x) > 0\Leftrightarrow x > - \frac{1}{\ln 2}, f'(x) <   0\Leftrightarrow x <   - \frac{1}{\ln 2},$
