...
<span class="fake-tag">解析</span>求双层的函数值，先计算里面那一层，由题意， $f(1) = 1^{2} + 2 = 3$ ，所以 $f(f(1)) = f(3) = 3 - 1 = 2$ .

<span class="fake-tag">答案</span>: 2

##### 【变式1】已知函数 $f(x) = \begin{cases} x - 1, & x > 2 \\ x^2 + 2, & x \leq 2 \end{cases}$ ，若 $f(a) = 3$ ，则 $f(a - 1) =$ \_\_\_\_.

<span class="fake-tag">解析</span>想解方程 $f(a) = 3$ ，但 $f(a)$ 该代哪个解析式？这是由 $a$ 与2的大小关系决定的，所以分类讨论，当 $a > 2$ 时，则 $f(a) = a - 1 = 3$ ，解得： $a = 4$ ，所以 $f(a - 1) = f(3) = 3 - 1 = 2$ ；
...
<span class="fake-tag">答案</span>: 6 或 2
...
<span class="fake-tag">解析</span>看到复合结构的方程，常考虑将内层的 $f(x)$ 换元，化整为零，
...
<span class="fake-tag">答案</span>: 4 或 $\pm 1$
...
<span class="fake-tag">解析</span>分段函数整体单调，分别考虑每一段的单调性，以及间断点处的拼接情况即可，
...
<span class="fake-tag">解析</span> $\{a_{n}\}$ 是递增数列 $\Rightarrow a_{n} < a_{n + 1}$ 对任意的 $n\in \mathbf{N}^*$ 恒成立 $\Rightarrow f(n) < f(n + 1)$ 恒成立，
