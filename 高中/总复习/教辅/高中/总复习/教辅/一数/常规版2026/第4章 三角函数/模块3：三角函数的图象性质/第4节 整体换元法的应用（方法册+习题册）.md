...
<span class="fake-tag">解析</span>由题意， $f(x) = \sin \left(\frac{\pi}{2} -x\right)\sin x - \sqrt{3}\cos^2 x = \cos x\sin x - \sqrt{3}\cos^2 x = \frac{1}{2}\sin 2x - \sqrt{3}\cdot \frac{1 + \cos 2x}{2}$
...
<span class="fake-tag">解析</span>由题意， $f(x) = 2\sqrt{3}\left(\sin 2x\cos \frac{\pi}{3} +\cos 2x\sin \frac{\pi}{3}\right) + \sqrt{3}\sin 2x - \cos 2x = 2\sqrt{3}\sin 2x + 2\cos 2x = 4\sin \left(2x + \frac{\pi}{6}\right),$ 所以 $f(x)\leq 2\sqrt{3}$ 即为 $4\sin \left(2x + \frac{\pi}{6}\right)\leq 2\sqrt{3}$ ，也即 $\sin \left(2x + \frac{\pi}{6}\right)\leq \frac{\sqrt{3}}{2}$
...
<span class="fake-tag">答案</span> $\left[k\pi +\frac{\pi}{4},k\pi +\frac{13\pi}{12}\right](k\in \mathbf{Z})$
...
<span class="fake-tag">解析</span>: 函数 $f(x) = \sin \left(2 x - \frac {\pi}{3}\right)$ 是由 $y = \sin t$ 和 $t = 2 x - \frac {\pi}{3}$ 复合而成, 复合函数遵循同增异减的单调性准则, 内层的 $t = 2 x - \frac {\pi}{3}$ 显然 $\nearrow$ , 所以要求 $f(x)$ 的增区间, 只需求 $y = \sin t$ 的增区间,
...
<span class="fake-tag">答案</span> $\left[k\pi -\frac{\pi}{12},k\pi +\frac{5\pi}{12}\right](k\in \mathbf{Z})$ ， $\left[k\pi +\frac{5\pi}{12},k\pi +\frac{11\pi}{12}\right](k\in \mathbf{Z})$
...
<span class="fake-tag">答案</span> $\left[k\pi +\frac{5\pi}{12},k\pi +\frac{11\pi}{12}\right](k\in \mathbf{Z})$

##### 【变式2】函数 $f(x) = \sin \left( 2x - \frac{\pi}{3} \right)$ 在 $\left[ -\frac{\pi}{2}, 0 \right]$ 上的单调递增区间是 \_\_\_\_.

<span class="fake-tag">解析</span>设 $t = 2x - \frac{\pi}{3}$ ，则 $f(x) = \sin t$ ，当 $x\in \left[-\frac{\pi}{2},0\right]$ 时， $t\in \left[-\frac{4\pi}{3}, - \frac{\pi}{3}\right]$
...
<span class="fake-tag">解析</span>先求 $f(x)$ 的单调区间再分析较麻烦，可考虑将 $\omega x - \frac{\pi}{3}$ 换元成 $t$ ，借助 $y = \sin t$ 的图象来研究问题，
...
<span class="fake-tag">答案</span> $\left(0,\frac{5}{2}\right]$
...
<span class="fake-tag">解析</span>本题若用辅助角公式合并成 $\sqrt{a^2 + 4}\sin (x + \varphi)$ ，由于 $\varphi$ 与 $a$ 有关，将 $x + \varphi$ 整体换元作用不大，不易分析单调性，所以不合并，那如何研究单调性呢？可求导分析，

由题意， $f'(x) = a\cos x - 2\sin x\leq 0$ 在 $\left[-\frac{\pi}{3}, - \frac{\pi}{4}\right]$ 上恒成立，所以 $a\cos x\leq 2\sin x$
...
<span class="fake-tag">答案</span> $(- \infty, -2 \sqrt{3} ]$
...
<span class="fake-tag">解析</span> $f(x) = \sin \left(2x + \frac{\pi}{4}\right)$ 的对称轴处函数值为 $\pm 1$ ，即 $\sin \left(2x + \frac{\pi}{4}\right) = \pm 1$ ，令 $t = 2x + \frac{\pi}{4}$ ，则 $\sin t = \pm 1$
...
<span class="fake-tag">答案</span> $x = \frac{k\pi}{2} +\frac{\pi}{8} (k\in \mathbf{Z})$ ， $\left(\frac{k\pi}{2} -\frac{\pi}{8},0\right)(k\in \mathbf{Z})$
...
$\quad$A. 关于点 $\left(-\frac{5\pi}{3}, 0\right)$ 对称 
$\quad$B. 关于点 $\left(\frac{\pi}{2}, 0\right)$ 对称

$\quad$C. 关于直线 $x = -\frac{2\pi}{3}$ 对称 
$\quad$D. 关于直线 $x = \frac{\pi}{2}$ 对称

<span class="fake-tag">解析</span>要判断选项，需求出 $g(x)$ 的解析式， $g(x)$ 是由 $f(x)$ 平移得来的，所以先求 $f(x)$ 的解析式，
...
<span class="fake-tag">解析</span>先把题干所给的零点和对称轴这两个条件翻译出来，建立关于 $\omega$ 的方程组，
...
<span class="fake-tag">解析</span>为了便于分析，先将 $\omega x - \frac{\pi}{4}$ 换元成 $t$ ，利用 $y = \sin t$ 的图象来研究零点和极值点，
...
<span class="fake-tag">答案</span> $\left(\frac{3}{4},\frac{5}{4}\right]$
...
$\quad$A. $\left[k\pi -\frac{\pi}{8},k\pi +\frac{3\pi}{8}\right](k\in \mathbf{Z})$   
$\quad$B. $\left[2k\pi -\frac{\pi}{8},2k\pi +\frac{3\pi}{8}\right](k\in \mathbf{Z})$   
$\quad$C. $\left[2k\pi + \frac{3\pi}{8}, 2k\pi + \frac{7\pi}{8}\right](k \in \mathbf{Z})$   
$\quad$D. $\left[k\pi + \frac{3\pi}{8}, k\pi + \frac{7\pi}{8}\right](k \in \mathbf{Z})$
...
$\quad$A. $\left[k\pi -\frac{\pi}{3},k\pi +\frac{\pi}{6}\right](k\in \mathbf{Z})$   
$\quad$B. $\left[k\pi + \frac{5\pi}{12}, k\pi + \frac{11\pi}{12}\right](k \in \mathbf{Z})$   
$\quad$C. $\left[k\pi -\frac{\pi}{12},k\pi +\frac{5\pi}{12}\right](k\in \mathbf{Z})$   
...
$\quad$B. 函数 $f(x)$ 的定义域为 $\left\{x \mid x \neq \frac{k\pi}{2} + \frac{5\pi}{12}, k \in \mathbf{Z}\right\}$   
$\quad$C. 点 $\left(\frac{5\pi}{12}, 0\right)$ 是函数 $f(x)$ 图象的一个对称中心  
$\quad$D. $f(x)$ 的单调增区间是 $\left(\frac{k\pi}{2} -\frac{\pi}{6},\frac{k\pi}{2} +\frac{\pi}{3}\right)(k\in \mathbf{Z})$
...
$\quad$A. $A = 2$   
$\quad$B. $f(x)$ 的最小正周期为 $\pi$

$\quad$C. $f(x)$ 在 $\left(-\frac{5\pi}{12}, \frac{5\pi}{6}\right)$ 内有3个极值点

$\quad$D. $f(x)$ 在区间 $\left[\frac{11\pi}{6}, 2\pi\right]$ 上的最大值为 $\sqrt{3}$
...
$\quad$C. $\left[\frac{1}{2},\frac{3}{4}\right]$

$\quad$D. $\left[\frac{1}{2},\frac{5}{4}\right]$
...
$\quad$A. $\left[\frac{\pi}{9},\frac{7\pi}{18}\right]$

$\quad$B. $\left[\frac{2\pi}{9}, \frac{7\pi}{18}\right]$

$\quad$C. $\left[\frac{\pi}{9},\frac{5\pi}{18}\right]$

$\quad$D. $\left[\frac{2\pi}{9}, \frac{5\pi}{18}\right]$

##### 16. (2023·山东济南二模·★★★☆)

已知函数 $f(x) = a\sin 2x + b\cos 2x (ab \neq 0)$ 的图象关于直线 $x = \frac{\pi}{6}$ 对称，则下列说法正确的是（）

$\quad$A. $f\left(x - \frac{\pi}{6}\right)$ 是偶函数  
$\quad$B. $f(x)$ 的最小正周期为 $2 \pi$   
$\quad$C. $f(x)$ 在区间 $\left[-\frac{\pi}{3},\frac{\pi}{6}\right]$ 上单调递增  
$\quad$D. 方程 $f(x) = 2b$ 在区间 $[0, 2\pi]$ 上有 2 个实根
