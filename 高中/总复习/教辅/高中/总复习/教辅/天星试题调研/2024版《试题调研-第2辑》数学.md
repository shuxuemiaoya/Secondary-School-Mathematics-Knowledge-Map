...
Yellow star-shaped mascot wearing a blue patterned outfit with stars, posing with one hand raised (no text or symbols visible)
</details>
...
| Point | x (log [c(Cl⁻)/(mol·L⁻¹)]) | y (-lg [c(Ag)²⁺/(mol·L⁻¹)]) |
|-------|-----------------------------|------------------------------|
| a     | 4                           | 6                            |
| b     | 8                           | 2                            |
</details>

C. $Ag_{2}CrO_{4} + 2Cl^{-} \rightleftharpoons 2AgCl + CrO_{4}^{2-}$ 的平衡常数 $K=10^{7.9}$
...
D. 反应 $\mathrm{Ag_{2}CrO_{4}(s)+2Cl^{-}(aq)}\rightleftharpoons2\mathrm{AgCl(s)}+\mathrm{CrO_{4}^{2-}(aq)}$ 的平衡常数 $K=10^{8.2}$

# 【生物·第8辑·P99】

人体水盐平衡的调节离不开抗利尿激素（ADH）和醛固酮的作用。……

A. 抗利尿激素的受体位于细胞膜上，当血浆渗透压升高时，抗利尿激素分泌量增加
...
B. $\triangle GAB$ 面积的最小值是 $\frac{2}{3}$
...
为2, $B$ 的轨迹为阿波罗尼斯圆,如图3所示. 这为使用坐标法解决向量问题指明了方向, $\left| c - a\right|  + \left| {c + a}\right|  = 4$ 则更为直接,可理解为动点 $C$ (满足 $\overrightarrow{OC} = c$ )到定点 $A$ 和定点 ${A}^{\prime }$ (满足 $\overrightarrow{O{A}^{\prime }} =  - a$ ) 的距离之和为定值,所以 $C$ 的轨迹为以 $A,{A}^{\prime }$ 为焦点, $a = 2$ 的椭圆.
...
C. $|\overrightarrow{AG}| \geqslant 1$
...
Illustration of a waterwheel with a potted plant and scattered leaves, no text or symbols present
</details>
...
A. 筒车转动的角速度 $\omega = \frac{\pi}{50}$   
B. 当 t = 75 秒时, 点 P 的纵坐标为 -2  
C. 当 t = 75 秒时, 点 P 和初始点 $P_{0}$ 的距离为 4  
D. 当 $0 < t \leqslant 30$ 时, 点 P 与 x 轴的最大距离为 4
...
B. 在第 10 分钟和第 40 分钟时点 $P$ 距离地面的高度相同  
C. 摩天轮旋转一周的过程中, 点 P 距离地面的高度不低于 55 米的时间大于 20 分钟  
D. 点 $P$ 从第 5 分钟至第 10 分钟上升的高度是其从第 10 分钟到第 15 分钟上升的高度的 2 倍
...
C. $(\overrightarrow{AC} \times \overrightarrow{AB}) \cdot \overrightarrow{AD} = 4\sqrt{2}$
...
A. 设 $\boldsymbol{a} = (m, n)$ ，则 $\left|\boldsymbol{a}\right| = \sqrt{m^{2} + n^{2} + 2mn\cos\alpha}$   
B. 设 $\boldsymbol{a} = (m, n)$ , $\boldsymbol{b} = (s, t)$ , 若 $a \parallel b$ , 则 mt - ns = 0   
C. 设 $\boldsymbol{a}=(m,n)$ , $\boldsymbol{b}=(s,t)$ , 若 $a\perp b$ , 则 $ms+nt+(mt+ns)\sin\alpha=0$   
D. 设 $a = (-1, 2)$ , $b = (-2, 1)$ , 若 a 与 b 的夹角为 $\frac{\pi}{3}$ , 则 $\alpha = \frac{\pi}{3}$
...
Yellow cartoon star character wearing glasses and a outfit with stars (no text or symbols)
</details>
...
C. $\angle MPN$ 的余弦值为 $\frac{\sqrt{21}}{21}$

D. $\overrightarrow{PA} + \overrightarrow{PB} + \overrightarrow{PC} = 0$
...
Exterior view of a multi-tiered traditional Chinese pagoda (no signage or text visible)
</details>
...
Stylized cartoon star character with large eyes and a smiling face, wearing orange outfit (no text or symbols)
</details>
...
```mermaid
graph LR
    A["由题意得 sin(α+α+β) = cos(α+β-α)"] --> B["利用两角和与差的正、余弦公式展开"]
    B --> C["两边同时除以 cos α"]
    D["tan α=2"] --> E["化简变形得 tan(α+β)=1"]
    E --> F["利用两角差的正切公式即可得解"]
```
</details>
...
A. $f(x)$ 的最大值为2  
B. $f(x)$ 在 $\left[-\frac{\pi}{8},\frac{\pi}{12}\right]$ 上单调递增  
C. $f(x)$ 在 $[0, \pi]$ 上有 4 个零点  
D. 把 $f(x)$ 的图象向右平移 $\frac{\pi}{12}$ 个单位长度, 得到的图象关于直线 $x = -\frac{\pi}{8}$ 对称
...
B. 函数 $f(x)$ 的最小正周期为 $\pi$   
C. 函数 $f(x)$ 的图象的对称轴为直线 $x = k\pi + \frac{\pi}{12} (k \in \mathbf{Z})$   
D. 函数 $f(x)$ 的图象可由 $y = \cos 2x$ 的图象向左平移 $\frac{\pi}{12}$ 个单位长度得到
...
A. $f(x) = \sqrt{3} \sin\left(\frac{\pi}{4}x - \frac{3\pi}{4}\right)$

B. $f(x) = \sqrt{3}\sin \left(\frac{\pi}{4} x + \frac{\pi}{4}\right)$

C. 点(2023,0)是 $f(x)$ 图象的一个对称中心

D. 函数 $f(x)$ 的图象向左平移 $\frac{\pi}{4}$ 个单位长度得到的图象关
...
A. $f(x)$ 在区间 $(- \frac{\pi}{9}, \frac{\pi}{9})$ 上单调递增  
B. $f(x)$ 在区间 $(-π,0)$ 内有三个零点  
C. 直线 $x = \frac{11\pi}{18}$ 是曲线 $y = f(x)$ 的对称轴

D. 将 $f(x)$ 的图象向左平移 $\frac{\pi}{3}$ 个单位长度, 所得图象对应的函数为奇函数
...
C. 将 $f(x)$ 的图象向右平移 $\frac{\pi}{12}$ 个单位长度所得图象对应的函数为奇函数

D. $f(x)$ 的单调递增区间为 $\left[-\frac{\pi}{12}+\frac{2k\pi}{3},\frac{\pi}{4}+\frac{2k\pi}{3}\right](k\in\mathbf{Z})$
...
A. $f(x)$ 的最小正周期为 $4\pi$   
B. $f(\frac{2\pi}{9}) > f(\frac{10\pi}{9})$   
C. 将 $f(x)$ 的图象向右平移 $\frac{4\pi}{3}$ 个单位长度后所得图象对应的函数为偶函数  
D. 函数 $y = 5f(x) + 4$ 在 $[0, \pi]$ 上有且仅有一个零点
...
A. 向右平移 $\frac{\pi}{5}$ 个单位长度

B. 向左平移 $\frac{\pi}{5}$ 个单位长度

C. 向右平移 $\frac{2 \pi}{5}$ 个单位长度

D. 向左平移 $\frac{2 \pi}{5}$ 个单位长度
...
A. 向右平移 $\frac{\pi}{12}$ 个单位长度, 再向上移动 1 个单位长度  
B. 向左平移 $\frac{\pi}{4}$ 个单位长度, 再向上移动 1 个单位长度  
C. 向右平移 $\frac{\pi}{12}$ 个单位长度, 再向下移动 1 个单位长度

# 趣味数学

D. 向右平移 $\frac{\pi}{4}$ 个单位长度, 再向下移动 1 个单位长度
...
A. 横坐标伸长到原来的 2 倍, 再向左平移 $\frac{\pi}{4}$ 个单位长度  
B. 横坐标伸长到原来的 2 倍, 再向左平移 $\frac{\pi}{8}$ 个单位长度  
C. 向左平移 $\frac{\pi}{8}$ 个单位长度, 再把横坐标伸长到原来的 2 倍  
D. 向右平移 $\frac{\pi}{4}$ 个单位长度, 再把横坐标缩短到原来的 $\frac{1}{2}$
...
A. 先向右平移 $\frac{2 \pi}{3}$ 个单位长度, 再向上平移 1 个单位长度  
B. 先向右平移 $\pi$ 个单位长度, 再向上平移 4 个单位长度  
C. 先向右平移 $2 \pi$ 个单位长度, 再向上平移 1 个单位长度  
D. 先向右平移 $\frac{4 \pi}{9}$ 个单位长度, 再向上平移 4 个单位长度
...
A. $\left[-\frac{\sqrt{2}}{2},\sqrt{2}\right]$
...
Yellow cartoon star character wearing a outfit with stars and shorts (no text or symbols)
</details>
...
A. $|PQ|$ 的最小值为 $2 \sqrt{2}$   
B. $\overrightarrow{PC} \cdot \overrightarrow{PQ} \in [6,8]$   
C. $\overrightarrow{CP} \cdot \overrightarrow{CQ}$ 的最大值为 -2  
D. 过点 C 作直线 l 的垂线, 垂足为点 B, 则点 B 在某个定圆上
...
Stylized cartoon character with a yellow star and orange eyes, wearing a outfit with stars (no text or symbols)
</details>
...
A. $a_{n} = 2 \times \left(\frac{1}{3}\right)^{n - 2}$

B. $c_{5}=\frac{256}{9}$

C. 在 $\Omega_{3}$ 中, $\overrightarrow{OA} \cdot \overrightarrow{OC} = \overrightarrow{OD} \cdot \overrightarrow{OC}$

D. 在 $\Omega_{3}$ 中, $\overrightarrow{OB} \cdot \overrightarrow{OC} = 40$
...
A. 存在点 P, 使得 $\cos \angle F_{1}PF_{2} = -\frac{\sqrt{2}}{2}$   
B. 满足 $\triangle PF_{1}F_{2}$ 为直角三角形的点 $P$ 有 4 个  
C. 直线 PA 与直线 PB 的斜率乘积为定值 $-\frac{1}{3}$

D. 椭圆 C 内接矩形的周长的取值范围是 $(4,8]$
...
Yellow cartoon character with a star-like head and glasses, holding an object (no text or symbols)
</details>
...
Yellow cartoon character with large eyes and a smiling face, wearing a patterned outfit (no text or symbols)
</details>
...
Yellow cartoon star character with large eyes and a smiling face, wearing a outfit with stars (no text or symbols)
</details>
...
A. 若 $\lambda + \mu = 1$ , 则点 $P$ 的轨迹不可能经过 $\triangle ABC$ 的外心  
B. 若 $\lambda + \mu = 1$ , 则点 $P$ 的轨迹不可能经过 $\triangle ABC$ 的垂心  
C. 若 $\lambda + \mu = \frac{1}{2}$ , 则点 $P$ 的轨迹不可能经过 $\triangle ABC$ 的重心  
D. 若 $\lambda \in [0,1], \mu \in [0,1]$ , 则点 $P$ 的轨迹一定过 $\triangle ABC$ 的外心
...
Geometric diagram of a quadrilateral ABCD with no text or symbols
</details>
...
A. 函数 $g(x)$ 的最小正周期为 $\pi$   
B. 函数 $g(x)$ 在 $[0, \frac{\pi}{2}]$ 上单调递增  
C. 函数 $g(x)$ 的一个极值点为 $x=\frac{\pi}{12}$   
D. 函数 $g(x)$ 的一个零点为 $x = -\frac{\pi}{6}$
...
C. $\left[\frac{7}{6},\frac{5}{3}\right)$
...
A. $f(x)$ 的解析式为 $f(x) = \sqrt{3}\sin (2x + \frac{\pi}{3}) - 1$

B. $f(\frac{\pi}{3}) + f(\frac{2\pi}{3}) + f(\frac{3\pi}{3}) + \cdots + f(\frac{2023\pi}{3}) = 2023$

C. 若 $f(x)$ 在 $[-m, m]$ 上单调递增，则 $0 < m \leqslant \frac{\pi}{12}$

D. 若将 $f(x)$ 图象上每个点的横坐标变为原来的 $\frac{1}{t}(t>0)$ 后, 所得图象在 $[0, \frac{\pi}{2}]$ 上恰有 4 个最高点, 则 $\frac{37}{6} \leq t < \frac{49}{6}$
...
C. $\frac{7\sqrt{3}}{8}$ D. $\frac{5\sqrt{3}}{8}$
...
Yellow star-shaped mascot with a red eye-like design and '100' text, wearing a blue patterned shirt and yellow tie (no readable text beyond the number)
</details>
...
Illustration of a calendar icon with a clock, no text or symbols present
</details>
