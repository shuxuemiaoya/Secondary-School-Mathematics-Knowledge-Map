...
<span class="fake-tag">解析</span> 令 $g(x) = \mathrm{e}^{x}f(x)$ , 则 $g'(x) = \mathrm{e}^{x}[f(x) + f'(x)] > 0$ , 所以 $g(x)$ 在 $\mathbb{R}$ 上单调递增,
...
<span class="fake-tag">解析</span> 构造函数 $f(x) = \frac{\ln x}{x + 0.1} (x > 0)$ ，则 $f'(x) = \frac{\frac{x + 0.1}{x} - \ln x}{(x + 0.1)^2}$ .
...
<span class="fake-tag">解析</span> （1） 由已知得 $a = 2, \frac{b}{c} = \sqrt{3}$ , 即 $b = \sqrt{3} c$ .
...
<span class="fake-tag">解析</span> 由 $\mathrm{e}^x > 1 + x(x > 0), \ln (1 + x) < x(x > 0)$ , 知 $\mathrm{e}^x - 1 > x > \ln (1 + x)(x > 0)$ ,
...
<span class="fake-tag">解析</span> （1） $f'(x) = 6\cos 2x + 6\cos 3x\cos x - 2\sin 3x\sin x + a = 6\cos 2x + 4(\cos 3x\cos x - \sin 3x\sin x) + 2(\cos 3x\cos x + \sin 3x\sin x) + a = 8\cos 2x + 4\cos 4x + a = 8\cos 2x + 8\cos^{2}2x - 4 + a = 2(2\cos 2x + 1)^{2} + (a - 6)$ ,
...
<span class="fake-tag">解析</span> 由 $f(x)=x+\ln(4-x)$ ，知 $f(x)$ 的定义域为 $(-∞,4)$ ，且 $f'(x)=1+\frac{-1}{4-x}=\frac{3-x}{4-x}$
...
<span class="fake-tag">解析</span> （1） $f(x) = \cos \pi x - \sin \pi x = \sqrt{2} \cos (\pi x + \frac{\pi}{4})$
...
$\quad$B. $f(x)=\log_{\frac{1}{2}}(2^{x}+1)+\frac{1}{2}x$ 不是圆 $x^{2}+(y+1)^{2}=1$ 的太极函数  
...
<span class="fake-tag">解析</span> 方法一 （代数法）由题意可知, 直线 l 过抛物线 C 的焦点 F, 设 $A(x_{1}, y_{1}) (x_{1} > 0$ ,
