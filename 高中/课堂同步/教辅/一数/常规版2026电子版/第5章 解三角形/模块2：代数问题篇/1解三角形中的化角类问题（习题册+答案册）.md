...
解析：已知 $B$ ，可求出 $A$ 和 $C$ 的关系，用来将所给的关于 $A$ 和 $C$ 的方程消元， $B = 150^{\circ} \Rightarrow A + C = 180^{\circ} - B = 30^{\circ}$
...
解析：因为 $m \parallel n$ ，所以 $(b + a + c)(b - a + c) = \sqrt{3}c \cdot \sqrt{3}b$ ，
...
Geometric diagram of triangle ABC with perpendicular lines and labeled points
</details>
