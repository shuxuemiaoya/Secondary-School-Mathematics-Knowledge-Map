...
解析：设 $f(m) = f(n) = t$ ，如图，由图可知 $0 \leq t < 1$ ，且 $m < 0 \leq n$ ，所以 $f(m) = m + 1 = t \Rightarrow m = t - 1$
...
B. $\left(-2,\frac{81}{10}\right]$

C. $\left(-2,\frac{61}{10}\right]$

D. $\left(0, \frac{81}{10}\right]$

解析：条件给的是函数 $y = |f(x)|$ 在 $a, b, c, d$ 处函数值相等，故用 $y = |f(x)|$ 的图象来分析问题，先画图，函数 $y = f(x)$ 的大致图象如图1， $y = |f(x)|$ 的大致图象如图2，设 $|f(a)| = |f(b)| = |f(c)| = |f(d)| = t$ ，
