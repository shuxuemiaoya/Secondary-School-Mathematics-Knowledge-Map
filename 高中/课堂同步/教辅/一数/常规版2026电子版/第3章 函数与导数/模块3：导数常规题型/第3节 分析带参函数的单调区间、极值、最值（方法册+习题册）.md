...
$$
f ^ {\prime} (x) \rightarrow \left\{\begin{array}{l l}\text {无零点}\\\text {有零点}\end{array}\right.\left\{\begin{array}{l l}\text {只有一个零点}\\\text {有多个零点} \rightarrow \text {讨论零点的大小}\end{array}\right.
$$
...
解：由题意， $f^{\prime}(x) = \frac{1}{x} -a = \frac{1 - ax}{x} (x > 0),$
...
解：由题意， $f^{\prime}(x) = x^{2} + (2a - 1)x - 2a = (x + 2a)(x - 1)$
...
若 $x > 0$ ，则 $\mathrm{e}^x -2a > 1 - 2a > 0$ ，所以 $f^{\prime}(x) > 0$
