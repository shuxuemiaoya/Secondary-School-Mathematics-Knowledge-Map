...
解析：所给不是抛物线的标准方程，应先化为标准方程，再观察焦点坐标，由 $y = 4x^{2}$ 可得 $x^{2} = \frac{1}{4} y$ ，
...
解析：没有规定开口，则过点 $A(2,2)$ 的抛物线有如图所示的两种情况，下面分别考虑，
...
解析：如图，直接分析 $y + |PQ|$ 的最小值不易，可考虑把 $y$ 凑成 $y + 1$ ，用定义转化为 $|PF|$ 再看，
...
解析：如图， $A$ ， $B$ 都是动点，不好分析，可先取定一个．由于圆上动点到圆外定点的距离最值比较好求，故先取定 $A$
