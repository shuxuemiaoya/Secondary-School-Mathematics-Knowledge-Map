...
[例3](1)数列 $\left\{a_{n}\right\}$ 的通项公式为 $a_{n}=\frac{1}{\sqrt{n+1}+\sqrt{n}}$ ，若该数列的前k项和等于5，则k=\_\_\_\_。(2)正项数列 $\left\{a_{n}\right\}$ 满足 $a_{n}^{2}+\sqrt{n}\cdot a_{n}-2n=0$ ，记[x]表示不超过x的最大整数，则 $\left[\sum_{i=1}^{100}\frac{1}{a_{i}}\right]=$ \_\_\_\_。
