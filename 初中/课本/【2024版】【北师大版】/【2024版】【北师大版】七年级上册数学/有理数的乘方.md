...
经过 $5 \mathrm{~h}$ 分裂 10 次，分裂成： $\overbrace{2 \times 2 \times \cdots \times 2 \times 2}^{10\text{ 个 }2} = 1024 \text{（个）}.$
...
$$
一般地，求 $n$ 个相同因数 $a$ 的积的运算叫作[乘方](概念/乘方.md)（power），乘方的结果叫作[幂](概念/幂.md)（power），$a$ 叫作[底数](概念/底数.md)（base number），$n$ 叫作[指数](概念/指数.md)（exponent）。$a^{n}$ 读作"$a$ 的 $n$ 次幂"（或"$a$ 的 $n$ 次方"）。
<div align="center">
