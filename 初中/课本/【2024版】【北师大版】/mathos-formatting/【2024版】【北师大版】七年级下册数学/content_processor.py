...
def apply_image_caption_fixes(text: str) -> str:
    ...
    def convert_image_table(m):
        ...
        for line in lines:
            ...
            if line.startswith('!['):
                images.append(line)
            elif line.startswith('图') or line.startswith('（') or line.startswith('('):
                captions.append(line)
        if len(images) == len(captions) and len(images) > 1:
            # 构建表格
            table = '| '
            table += ' | '.join(images) + ' |\n| '
            table += ' | '.join([':---:' for _ in images]) + ' |\n| '
            table += ' | '.join(captions) + ' |'
            return table
        elif len(images) == 1 and len(captions) == 1:
            # 单张图片+题注，居中显示
            return f'<center>{images[0]}</center>\n<center>{captions[0]}</center>'
        return content
    # 匹配连续图片和题注块
    text = re.sub(r'(?:!\[[^\]]*\]\([^\)\n]+\)\s*\n\s*(?:图\s*\d+(?:\.\d+)*(?:-\d+)?|[（(][^）)\r\n]+[）)])\s*\n?)+', convert_image_table, text)
    # 图片空行清理
    text = re.sub(r'(?m)(?:\r?\n[ \t]*)+(?=> (?:\||<center>))', '\n', text)
    text = re.sub(r'(?m)(?:\r?\n[ \t]*)+(?=<center>(?:!\[|<img))', '\n', text)
    text = re.sub(r'(?:\r?\n[ \t]*)+(?=<center>)', '', text)
...
def apply_blank_line_fixes(text: str) -> str:
    ...
    # 数学推导词白名单：删除这些行前多余空行
    math_keywords = r'(解：|列方程|综上所述|根据题意|依题意|由题意|由已知|由条件|据题意|据已知|由此可知|由此可得|也就是说|同理可得|分类讨论|整理得|化简得|经检验|所以|因为|因此|于是|从而|∵|∴|显然|证明|欲证|要证|解得|可得|可知|代入|联立|移项|合并|消去|设|若|则|得|故|当|由|又|即|将|答|Rt)'
    text = re.sub(r'(?m)(\n{2,})(' + math_keywords + r')', r'\n\2', text)
    return text
