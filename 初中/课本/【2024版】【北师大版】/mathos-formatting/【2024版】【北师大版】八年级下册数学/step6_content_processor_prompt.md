...
3. 单张图片 + `图1.2` / `（第3题）` → 保持 Markdown 原样格式并居中，严禁转换为 HTML <img> 标签（如：可以使用 `<center>![alt](path)</center>\n<center>题注</center>`）。必须保留标准的 Markdown 图片 `![alt](path)` 括号语法以通过自检。
4. 警告：在编写图片正则提取和替换时，必须注意 `!\[([^\]]*)\]\(([^\)\n]+)\)` 中，group(1) 是图片的 alt 文本，group(2) 才是图片的 URL/路径！严禁混淆它们导致 src 变为 alt 或者丢失！
5. 不改变图片路径、顺序、题注顺序。
6. 复杂图片处理使用辅助函数，不要堆在一个巨大正则里。
