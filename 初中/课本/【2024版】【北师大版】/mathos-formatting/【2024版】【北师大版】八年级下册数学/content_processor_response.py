...
def apply_image_caption_fixes(text: str) -> str:
    ...
    # 处理连续图片和题注
    # 先处理单张图片+题注
    def single_image_caption(match):
        img = match.group(1)
        cap = match.group(2)
        return f"<center>{img}</center>\n<center>{cap}</center>"
    text = re.sub(r'(?m)^(!\[[^\]]*\]\([^\)\n]+\))\s*\n\s*(图\s*\d+(?:\.\d+)*(?:-\d+)?)', single_image_caption, text)
    text = re.sub(r'(?m)^(!\[[^\]]*\]\([^\)\n]+\))\s*\n\s*([（(][^）)\r\n]+[）)])', single_image_caption, text)
    # 处理多张连续图片+等量连续题注 -> 图片表格
    # 使用辅助函数处理
    text = convert_image_groups_to_table(text)
    # 图片空行清理
    text = re.sub(r'(?m)(?:\r?\n[ \t]*)+(?=> (?:\||<center>))', '\n', text)
    text = re.sub(r'(?m)(?:\r?\n[ \t]*)+(?=<center>(?:!\[|<img))', '\n', text)
    text = re.sub(r'(?:\r?\n[ \t]*)+(?=<center>)', '', text)
...
def convert_image_groups_to_table(text: str) -> str:
    """将多张连续图片 + 等量连续题注转换为 Markdown 图片表格。"""
    lines = text.split('\n')
    result = []
    i = 0
    while i < len(lines):
        ...
        if re.match(r'^\s*!\[[^\]]*\]\([^\)\n]+\)\s*$', lines[i]):
            ...
            while i < len(lines) and re.match(r'^\s*!\[[^\]]*\]\([^\)\n]+\)\s*$', lines[i]):
                ...
                i += 1
            # 收集连续题注
            while i < len(lines) and re.match(r'^\s*(图\s*\d+(?:\.\d+)*(?:-\d+)?|[（(][^）)\r\n]+[）)])\s*$', lines[i]):
                cap_lines.append(lines[i].strip())
                i += 1
            if len(img_lines) == len(cap_lines) and len(img_lines) > 1:
                # 构建表格
                table = "| 图片 | 题注 |\n| --- | --- |\n"
                for img, cap in zip(img_lines, cap_lines):
                    table += f"| {img} | {cap} |\n"
                result.append(table)
            elif len(img_lines) == 1 and len(cap_lines) == 1:
                # 单张图片+题注，保持居中格式
                result.append(f"<center>{img_lines[0]}</center>\n<center>{cap_lines[0]}</center>")
            else:
                # 不匹配，原样输出
...
def apply_blank_line_fixes(text: str) -> str:
    ...
    # 数学推导词白名单空行删除
    math_words = [
        '解：', '列方程', '综上所述', '根据题意', '依题意', '由题意', '由已知', '由条件',
        '据题意', '据已知', '由此可知', '由此可得', '也就是说', '同理可得', '分类讨论',
        '整理得', '化简得', '经检验', '所以', '因为', '因此', '于是', '从而', '∵', '∴',
        '显然', '证明', '欲证', '要证', '解得', '可得', '可知', '代入', '联立', '移项',
        '合并', '消去', '设', '若', '则', '得', '故', '当', '由', '又', '即', '将', '答', 'Rt'
    ]
