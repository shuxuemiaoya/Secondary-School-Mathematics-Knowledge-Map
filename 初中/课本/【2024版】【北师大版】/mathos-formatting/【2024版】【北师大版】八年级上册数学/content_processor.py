...
def protect_blocks(text: str) -> tuple[str, list[str]]:
    ...
    blocks = []
    # 保护 YAML frontmatter
    text = re.sub(r'^---\n[\s\S]*?\n---\n', lambda m: f'<!--YAML_BLOCK_{len(blocks)}-->\n', text)
    # 保护 fenced code blocks
    text = re.sub(r'```[\s\S]*?```', lambda m: f'<!--CODE_BLOCK_{len(blocks)}-->', text)
    # 保护行内代码
    text = re.sub(r'`[^`\n]+`', lambda m: f'<!--INLINE_CODE_{len(blocks)}-->', text)
    # 保护行间公式
    text = re.sub(r'\$\$[\s\S]*?\$\$', lambda m: f'<!--DISPLAY_MATH_{len(blocks)}-->', text)
    # 保护行内公式
    text = re.sub(r'\$[^\$]*?\$', lambda m: f'<!--INLINE_MATH_{len(blocks)}-->', text)
    return text, blocks

def restore_blocks(text: str, blocks: list[str]) -> str:
    """恢复被保护内容。"""
    # 恢复行内公式
    text = re.sub(r'<!--INLINE_MATH_(\d+)-->', lambda m: blocks[int(m.group(1))], text)
    # 恢复行间公式
    text = re.sub(r'<!--DISPLAY_MATH_(\d+)-->', lambda m: blocks[int(m.group(1))], text)
    # 恢复行内代码
    text = re.sub(r'<!--INLINE_CODE_(\d+)-->', lambda m: blocks[int(m.group(1))], text)
    # 恢复代码块
    text = re.sub(r'<!--CODE_BLOCK_(\d+)-->', lambda m: blocks[int(m.group(1))], text)
    # 恢复 YAML
    text = re.sub(r'<!--YAML_BLOCK_(\d+)-->', lambda m: blocks[int(m.group(1))], text)
    return text
    ...
    while i < len(lines):
        if re.match(r'^!\[[^\]]*\]\([^\)\n]+\)$', lines[i]):
            ...
            while i < len(lines) and re.match(r'^!\[[^\]]*\]\([^\)\n]+\)$', lines[i]):
                ...
                i += 1
            # 收集连续题注
            while i < len(lines) and re.match(r'^(图\s*\d+(?:\.\d+)*(?:-\d+)?|[（(][^）)\r\n]+[）)])$', lines[i]):
                captions.append(lines[i])
                i += 1
            if len(images) > 1 and len(captions) == len(images):
                # 多张图片 + 等量题注 → 图片表格
                table_lines = []
                table_lines.append('| 图片 | 题注 |')
                table_lines.append('|------|------|')
                for img, cap in zip(images, captions):
                    table_lines.append(f'| {img} | {cap} |')
                # 替换原位置
                start = i - len(images) - len(captions)
                lines[start:i] = table_lines
                i = start + len(table_lines)
            elif len(images) == 1 and len(captions) == 1:
                # 单张图片 + 题注 → 居中
                img = images[0]
                cap = captions[0]
                start = i - 2
                lines[start:i] = [f'<center>{img}</center>', f'<center>{cap}</center>']
                i = start + 2
            elif len(images) > 1 and len(captions) == 0:
                # 多张图片无题注，保持原样
                pass
            else:
                # 其他情况，保持原样
...
def apply_blank_line_fixes(text: str) -> str:
    ...
    # 数学推导词白名单空行删除
    math_words = [
        '解：', '列方程', '综上所述', '根据题意', '依题意', '由题意', '由已知', '由条件', '据题意', '据已知',
        '由此可知', '由此可得', '也就是说', '同理可得', '分类讨论', '整理得', '化简得', '经检验', '所以', '因为',
        '因此', '于是', '从而', '∵', '∴', '显然', '证明', '欲证', '要证', '解得', '可得', '可知', '代入', '联立',
        '移项', '合并', '消去', '设', '若', '则', '得', '故', '当', '由', '又', '即', '将', '答', 'Rt'
    ]
