...
> [!example] 例题 46
> 已知函数 $f(x)=(ax-x^{2})\mathrm{e}^{x}(a\geqslant0)$ .
...
> [!done] 解答
> （1）由题意知 $f'(x) = (ax - x^2 + a - 2x)e^x \leqslant 0$ 在区间 $[2, +\infty)$ 上恒成立，即
...
> [!example] 例题 47
> 设函数 $f(x)=\frac{k}{3}x^{3}-\frac{1}{2}x^{2}-x, k \in \mathbb{R}.$
...
> [!done] 解答
> （1）由题意知 $g'(x) = \ln (x + 1) + kx^{2} - x\geqslant 0$ 在区间 $[0, + \infty)$ 上恒成立，令 $h(x) =$ $\ln (x + 1) + kx^{2} - x$ ，则
...
> [!example] 例题 49
> 已知函数 $f(x)=(x-a)\mathrm{e}^{x}-x+b$ ，曲线 $f(x)$ 在 x=0 处的切线方程为 y=-x.
> （1） 求 a, b 的值.  
...
> [!example] 例题51
> 已知函数 $f(x) = a(x - \ln x) + \frac{2x - 1}{x^2}, a \in \mathbb{R}$ .
> （1） 讨论 $f(x)$ 的单调性.  
...
> [!example] 例题52
> 设函数 $f(x) = \frac{\ln x}{(x - 1)^2}$ 在区间(0,1)上的极值点为 $x_0$ ，证明： $f(x_0) < -2$ .
...
> [!example] 例题 54
> 设 $a, b \in R$ ，已知函数 $f(x) = e^{-ax} + \frac{1}{\sqrt{x+1}} + b$ 在点 $(0, f(0))$ 处的切线方程为 $y = -\frac{3}{2}x$ .
> （1） 求 a, b 的值.  
...
> [!example] 例题55
> 已知函数 $f(x) = |x + 1| \mathrm{e}^{-\frac{1}{x}} - a$ .
...
> [!example] 例题 56
> 已知函数 $f(x)=ae^{x}-x^{2}(a\in\mathbb{R})$ ，若 $f(x)$ 有两个不同的极值点 $x_{1}, x_{2} (x_{1}<x_{2})$ .
> （1）求实数 a 的取值范围.  
> （2） 证明: $x_{2} < \frac{2}{a}$ .   
...
> [!example] 例题58
> 已知 $1 < a \leqslant 2$ ，函数 $f(x) = \mathrm{e}^{x} - x - a$ .
...
> [!example] 例题59
> 已知函数 $f(x) = \frac{\ln x}{x}$ ，直线 $l$ 为曲线 $y = f(x)$ 在点 $(t, f(t))$ 处的切线，直线 $l$ 与曲线 $y = f(x)$ 相交于点 $(s, f(s))$ 且 $s < t$ .
> （1） 求 t 的取值范围.  
...
> [!done] 解答
> （1） 对 $f(x)$ 求导得 $f'(x)=\frac{1-\ln x}{x^2}$ , 则
> $f ^ {'} (t) = \frac {1 - \ln t}{t ^ {2}}.$
>
...
> [!example] 例题 60
> 已知函数 $f(x)=\ln x+(1-a)x^{3}+bx$ , $g(x)=xe^{x}-b$ , 其中 $a, b \in R$ , 曲线 $f(x)$ 在点 $(e, f(e))$ 处的切线方程为 $y=\left(\frac{1}{e}+1\right)x$ .
> （1） 求 a, b 的值.  
...
> [!example] 例题62
> 已知函数 $f(x) = \frac{\mathrm{e}^x}{x} - \frac{a\ln\frac{x}{2}}{x^2} + x$ ，曲线 $y = f(x)$ 在点(2, $f(2)$ )处切线的斜率为 $\frac{\mathrm{e}^2}{4}$ .
> （1） 求 a 的值.  
> （2） 证明: $f(x) > e + 2$ .
...
> [!example] 例题65
> 已知函数 $f(x) = (\ln x)^2 - \frac{2\mathrm{e}^{x - 1}}{x}$ .
...
> [!example] 例题 66
> 已知正实数 x, y 满足方程 $(x+1)\left(\ln\frac{y}{x}+x+1\right)=2ye^{x}$ .
...
> [!example] 例题 67
> 当 $x \in (0,1) \cup (1,+\infty)$ 时，证明： $\frac{2}{e} < \frac{1}{x^{\frac{1}{x-1}}} + \frac{1}{x^{\frac{x}{x-1}}} < 1.$

> [!done] 证明
> 因为 $\frac{1}{x^{\frac{1}{x - 1}}} +\frac{1}{x^{\frac{x}{x - 1}}} = \frac{x + 1}{x^{\frac{x}{x - 1}}} = \mathrm{e}^{\ln (x + 1) - \frac{x}{x - 1}\ln x}$ ，所以只需证明
...
> [!example] 例题69
> 已知函数 $f(x) = \frac{\mathrm{e}^x - a}{x}$ ，其中 $x > 0$ .
> （1） 若函数 $f(x)$ 在区间 $(0, +\infty)$ 上单调递增，求 a 的取值范围.  
> （2） 当 a=1 时, 证明: 对任意 $m \leqslant 1$ , $f(x) > x \ln x + m$ .

> [!done] 解答
> （1）由题意知, $f'(x)=\frac{(x-1)e^{x}+a}{x^{2}}\geqslant0$ 在区间 $(0,+\infty)$ 上恒成立,即
...
> [!example] 例题 71
> 已知函数 $f(x)=xe^{x}-\ln x$ . ( $\ln 2 \approx 0.693, \sqrt{e} \approx 1.648$ , 均为不足近似值)
...
> [!done] 解答
> （1） 因为 $\ln x \leqslant x - 1 (x > 0)$ , 所以当 $a = 1$ 时,
...
> [!example] 例题 74
> 已知函数 $f(x)=\mathrm{e}^{x}(\sin x-ax^{2}+2a-\mathrm{e})$ .
...
> [!example] 例题 76
> 已知函数 $f(x)=\ln x+\frac{1}{2}x^{2}+ax(a\in\mathbb{R}), g(x)=\mathrm{e}^{x}+\frac{3}{2}x^{2}$ .
> （1） 讨论 $f(x)$ 的极值点的个数.  
...
> [!example] 例题 77
> 已知函数 $f(x)=\frac{1}{2}\mathrm{e}^{2x}+(1-a)\mathrm{e}^{x}-ax+\mathrm{e}$ .
> （1） 讨论函数 $f(x)$ 的单调性.  
...
> [!example] 例题 78
> 已知数列 $\{x_{n}\}(n\in\mathbb{N}^{*})$ 满足 $x_{n}>1$ ，记 $A=x_{1}x_{2}\cdots x_{n}, B=x_{1}+x_{2}+\cdots+x_{n}$ ， $C=\frac{1}{x_{1}}+\frac{1}{x_{2}}+\cdots+\frac{1}{x_{n}}$ ，证明： $A^{2}<e^{B-C}$ .
...
> [!example] 例题 79
> 已知定义在 R 上的函数 $f(x)=x^{2}+a\cos x+(a-2)e^{-x}$ .
> （1） 当 a=2 时, 求 $f(x)$ 的极值.  
