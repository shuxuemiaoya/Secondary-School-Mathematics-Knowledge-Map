...
def replace_in_file(path: Path) -> None:
    ...
    for index, line in enumerate(lines):
        ...
        if match:
            ...
            elif title_clean_no_space == "滴":
                matched_toc_key = "图说数学史 统计学点滴"
            ...
            # 3. 后缀匹配
            if not matched_toc_key:
                GENERIC_SUBHEADINGS = {"探究", "思考", "练习", "归纳", "数学活动", "小结", "复习巩固", "综合运用", "拓广探索", "习题", "活动", "任务", "复习题"}
                if title_clean_no_space not in GENERIC_SUBHEADINGS and len(title_clean_no_space) > 2:
